...
Both routes return one unsigned atomic PTB with exactly one public
`order_escrow::*_and_resolve_escrow` Move call. That wrapper binds the Case,
Bond, Config, and Escrow and performs decision plus payout in one indivisible
call; the underlying decision and binding helpers are package-private.

Do not call `/resolve-escrow` after a successful atomic PTB. IOTA retires that
route with `410 iota_dispute_resolve_escrow_route_retired` before auth, RPC, or
repository work. Only the separate Sui legacy lane retains binding-based
recovery. The platform wrapper follows the same one-call atomic rule but remains
an operator-only ArbCap path.
...
- any IOTA use of the retired `/resolve-escrow` route
