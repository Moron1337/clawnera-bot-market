...
- `apps/api/test/journeys/disputeReviewerFlow.test.ts`
  - invited reviewer accept / commit / reveal gating / replacement plus atomic
    one-wrapper finalize/fallback planning; IOTA resolve-escrow is retired
...
> Ziel dieses Dokuments: _Safety-first_ Abdeckung fuer alle geld-/governance-kritischen Pfade, inkl. Edge-Cases, Missbrauchsversuche und Indexer-Kompatibilitaet.
...
- `ESC-09a` Majority-, Timeout-Fallback- und Platform-Fallback-Builder erzeugen
  jeweils genau eine PTB mit genau einem passenden oeffentlichen
  `order_escrow::*_and_resolve_escrow`-Wrapper.
- `ESC-09b` Der Wrapper bindet exakte Case-, Bond-, Config- und Escrow-Inputs;
  fehlende, duplizierte oder umverdrahtete Inputs schlagen im Regressionstest
  fehl.
- `ESC-09c` Abort in einer Wrapper-Phase hinterlaesst weder eine nur
...
> Diese Tests sind _zusaetzlich_ zu A–J gedacht und sollen die typischen Audit-Failures abdecken (Entry-Surface, Timelock-Boundaries, Invariant/Soak, ABI/Event-Regressions).
...
- `GOV-17` Emergency Rotation darf **nicht** als „Timelock/2nd-Approver Bypass“ missbraucht werden.
  - Falls _by design_ doch: dokumentiere die Trust-Assumption und fuege Monitoring/Runbook hinzu.
...
> **Hinweis:** Das ist _Design-Optionen_ – nicht alles ist zwingend. Ich liste die typischen Luecken, die Audits/Incidents sonst spaeter teuer machen.
...
1. **Emergency Toggle durch Treasury** (nicht durch Admin), Rotation durch Admin _oder_ Treasury.
...
- _Primary Test_: der direkteste Negativtest fuer diesen Error-Code (kein "zufaelliges" Auftreten).
- _Secondary Tests_: weitere Stellen, wo der Error-Code ebenfalls erwartet wird (Regression-Backstop).

| Error-Code (E\_\*)                       | Primary Test | Secondary / Notes                                                                                     |
| ---------------------------------------- | ------------ | ----------------------------------------------------------------------------------------------------- |
...
| `E_SELF_DEAL_NOT_ALLOWED`                | `ESC-07`     | (auch fuer create*escrow*\* Varianten)                                                                |
...
2. Extrahiere erwartete Abort-Codes aus Tests (Move tests + TS/SDK tests), z.B. `assert_abort!(..., E_...)` oder "expect abort E\_...".
3. Vergleiche Mengen:
...
> Strategie: _fast deterministic gates_ fuer PR, _heavy + network gates_ nightly / pre-release.
