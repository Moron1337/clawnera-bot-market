...
function normalizeChainFamily(options = {}) {
  ...
  return normalized === "sui" ? "sui" : "iota";
}
...
function resolveSuiRpcUrl(options = {}) {
  ...
  if (explicit) {
    return { rpcUrl: explicit };
  }
  const envExplicit = String(process.env.CLAWNERA_SUI_RPC_URL || process.env.SUI_RPC_URL || "").trim();
  if (envExplicit) {
    return { rpcUrl: envExplicit };
  }
  const network = String(options.suiNetwork || options.chainNetwork || options.network || process.env.CLAWNERA_SUI_NETWORK || "").trim().toLowerCase();
  return { rpcUrl: network === "testnet" ? SUI_TESTNET_RPC_URL : SUI_MAINNET_RPC_URL };
}
...
function buildCreateOrderEscrowSuiTypedAssetTx(request) {
  ...
  const deadlineMs = assertPositiveU64(request?.deadlineMs, "deadline_ms");
  const feeConfigObjectId = assertSuiObjectId(request?.feeConfigObjectId, "fee_config_object_id");
  const coinType = normalizeSuiCoinType(request?.coinType || request?.usdcCoinType);
...
export function buildManagedStorageFeeTx(request) {
  if (isSuiRuntime(request) || request?.currency === SUI_NATIVE_SYMBOL || request?.currency === SUI_USDC_SYMBOL) {
    throw new Error("sui_managed_storage_fee_uses_api_tx_plan_bytes");
  }
...
export async function dryRunTransaction(transaction, options = {}) {
  if (isSuiRuntime(options)) {
    throw new Error("sui_direct_transaction_object_dry_run_unsupported_use_tx_plan_bytes");
  }
...
export async function executeTransaction(transaction, options = {}) {
  if (isSuiRuntime(options)) {
    throw new Error("sui_direct_transaction_object_execution_unsupported_use_tx_plan_bytes");
  }
  const built = await buildIotaTransactionBytes({ ...options, transaction });
