...
import { normalizeAuthenticatedUrl } from "./local-security.mjs";
import {
...
import { buildSuiPayManagedStorageFeeSuiTx } from "./vendor/clawdex-sdk/suiMilestoneManifestTx.js";
import {
...
const IOTA_FRESH_GOVERNED_TX_BUILDERS = new Set([
  "orderMailbox.init",
...
function assertIotaFreshGovernanceRequest(request) {
  if (!normalizeObjectId(request?.governanceConfigObjectId)) {
    throw new Error("iota_fresh_governance_config_required");
  }
}
...
function assertIotaFreshAtomicDisputeCloseout(tx, nativeFunction, typedFunction, typed) {
  const commands = tx?.getData?.()?.commands;
  const moveCall = Array.isArray(commands) && commands.length === 1 ? commands[0]?.MoveCall : null;
  const expectedFunction = typed ? typedFunction : nativeFunction;
  if (
    !moveCall ||
    moveCall.module !== "order_escrow" ||
    moveCall.function !== expectedFunction ||
    !Array.isArray(moveCall.arguments) ||
    moveCall.arguments.length !== 6
  ) {
...
function resolveSuiRpcUrl(options = {}) {
  const rawNetwork = String(
    options.suiNetwork || options.chainNetwork || options.network || process.env.CLAWNERA_SUI_NETWORK || "",
  ).trim().toLowerCase();
  ...
  if (explicit) {
    return { rpcUrl: normalizeAuthenticatedUrl(explicit, { errorCode: "invalid_sui_rpc_url" }), network };
  }
  const envExplicit = String(process.env.CLAWNERA_SUI_RPC_URL || process.env.SUI_RPC_URL || "").trim();
  if (envExplicit) {
    return { rpcUrl: normalizeAuthenticatedUrl(envExplicit, { errorCode: "invalid_sui_rpc_url" }), network };
  }
...
export function buildClawdexTxFromPlan(plan) {
  ...
  if (!isSuiPlan && IOTA_FRESH_GOVERNED_TX_BUILDERS.has(txBuilder)) {
    assertIotaFreshGovernanceRequest(request);
  }
  ...
  switch (txBuilder) {
    ...
    case "disputeQuorum.finalizeCase":
      return isSuiPlan
        ? buildSuiFinalizeDisputeCasePlanTx(request)
        : assertIotaFreshAtomicDisputeCloseout(
            buildFinalizeDisputeCaseTx(request),
            "finalize_case_with_quorum_and_resolve_escrow",
            "finalize_case_with_typed_quorum_and_resolve_escrow",
            request.bondCoinType !== undefined,
          );
    case "disputeQuorum.resolveFallback":
      throw new Error("admin_only_tx_builder:disputeQuorum.resolveFallback");
    case "disputeQuorum.resolveTimeoutFallback":
      return isSuiPlan
        ? buildSuiResolveDisputeTimeoutFallbackPlanTx(request)
        : assertIotaFreshAtomicDisputeCloseout(
            buildResolveDisputeTimeoutFallbackTx(request),
            "resolve_case_with_timeout_fallback_and_resolve_escrow",
            "resolve_case_with_typed_timeout_fallback_and_resolve_escrow",
            request.bondCoinType !== undefined,
          );
    case "orderEscrow.resolveDisputeWithBinding":
      ...
      return buildSuiResolveOrderEscrowWithBindingPlanTx(request);
    case "orderEscrow.resolveDisputeWithQuorumTicket":
      throw new Error("unsupported_order_escrow_quorum_ticket");
...
function requiredSuiEscrowCoinType(request) {
  try {
    return normalizeSuiCoinType(request?.escrowCoinType);
  } catch {
    throw new Error("invalid_escrow_coin_type");
  }
}
...
function buildSuiFinalizeDisputeCasePlanTx(request) {
  const { tx, packageId } = buildSuiBaseTx(request);
  const bondCoinType = suiBondCoinType(request);
  const disputeQuorumConfig = tx.object(
    assertSuiObjectId(request?.disputeQuorumConfigObjectId, "dispute_quorum_config_object_id"),
  );
  const escrowObjectId = assertSuiObjectId(request?.escrowObjectId, "escrow_object_id");
  const escrowCoinType = requiredSuiEscrowCoinType(request);
  tx.moveCall({
...
export function buildManagedStorageFeeTx(request) {
  const amount = request?.amount ?? request?.amountAtomic;
  const canonicalRequest = { ...request, amount };
  const currency = typeof request?.currency === "string" ? request.currency.trim().toUpperCase() : "";
  if (isSuiRuntime(request)) {
    if (currency && currency !== SUI_NATIVE_SYMBOL) {
      throw new Error("unsupported_sui_managed_storage_fee_currency");
    }
    return buildSuiPayManagedStorageFeeSuiTx(canonicalRequest);
  }
  if (currency && currency !== "IOTA") {
    throw new Error("unsupported_iota_managed_storage_fee_currency");
  }
  return buildPayManagedStorageFeeIotaTx(canonicalRequest);
}
...
export async function executeTransaction(transaction, options = {}, deps = {}) {
  if (isSuiRuntime(options)) {
    const built = await buildSuiTransactionBytes({ ...options, transaction });
  ...
  const built = await buildIotaTransactionBytes({ ...options, transaction }, deps);
...
export function assertDryRunSuccess(dryRunResult, prefix = "dry_run_failed") {
  const status =
    dryRunResult?.result?.effects?.status ??
    dryRunResult?.effects?.status ??
    dryRunResult?.result?.status ??
    dryRunResult?.status ??
    null;
  ...
  const label = typeof prefix === "string" && prefix.trim() ? prefix.trim() : "dry_run_failed";
  const failure = getExecutionFailure(dryRunResult);
  throw new Error(`${label}:${failure || "missing_success_status"}`);
}
