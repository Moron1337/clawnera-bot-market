import { createHash, scrypt as scryptCallback } from "node:crypto";
import fs from "node:fs/promises";
import os from "node:os";
import path from "node:path";
import { promisify } from "node:util";
import { xchacha20poly1305 } from "@noble/ciphers/chacha";
import { x25519 } from "@noble/curves/ed25519";
import { hkdf } from "@noble/hashes/hkdf";
import { sha256 } from "@noble/hashes/sha256";
import { randomBytes } from "@noble/hashes/utils";
import bs58 from "bs58";
import { isValidIotaAddress, isValidTransactionDigest, normalizeIotaAddress } from "@iota/iota-sdk/utils";
import {
  copyAndClearSensitiveBytes,
  readPrivateFile,
  writePrivateFileAtomicExclusive,
  writePrivateJsonAtomic,
} from "./local-security.mjs";

const BASE64URL_NO_PAD = /^[A-Za-z0-9\-_]+$/;
const KEY_LENGTH = 32;
const CEK_BYTES = 32;
const NONCE_BYTES = 24;
const WRAP_NONCE_BYTES = 24;
const WRAPPED_CEK_BYTES = CEK_BYTES + 16;
const MAX_DELIVERABLE_PLAINTEXT_BYTES = 16 * 1024 * 1024;
const MAX_DELIVERABLE_CIPHERTEXT_BYTES = MAX_DELIVERABLE_PLAINTEXT_BYTES + 16;
const WRAP_INFO_PREFIX = "clawdex:cek-wrap:v1";
const MANIFEST_SIGNING_PREFIX = "CLAWDEX Milestone Manifest v1";
const LEGACY_MANIFEST_SIGNING_INTENT_LINE = "intent=clawdex.milestone.submit.v1;chain=iota";
const DEFAULT_MANIFEST_SIGNING_INTENT = "clawdex.milestone.submit.v1";
const DEFAULT_MANIFEST_SIGNING_CHAIN = "iota";
const DEFAULT_MANIFEST_SIGNING_AUDIENCE = "clawdex-api";
const DEFAULT_MANIFEST_SIGNING_TTL_MS = 15 * 60 * 1000;
const MAX_MANIFEST_SIGNING_WINDOW_MS = 7 * 24 * 60 * 60 * 1000;
const MANIFEST_SIGNING_CONTEXT_TOKEN = /^[A-Za-z0-9._:-]+$/;
const MAX_MANIFEST_ORDER_ID_BYTES = 128;
const MAX_MANIFEST_MILESTONE_ID_BYTES = 128;
const MAX_MANIFEST_CID_BYTES = 512;
const MAX_MANIFEST_CIPHER_SUITE_BYTES = 128;
const MAX_MANIFEST_CONTEXT_INTENT_BYTES = 128;
const MAX_MANIFEST_CONTEXT_CHAIN_BYTES = 32;
const MAX_MANIFEST_CONTEXT_AUDIENCE_BYTES = 128;
const MAX_MANIFEST_CONTEXT_NONCE_BYTES = 128;
const SUPPLEMENTAL_RECIPIENT_MIN = 3;
const SUPPLEMENTAL_RECIPIENT_MAX = 64;
const UUID_PATTERN = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
const MANIFEST_RECIPIENT_HPKE_EPHEMERAL_BYTES = 32;
const MANIFEST_RECIPIENT_HPKE_NONCE_BYTES = 24;
const MANIFEST_RECIPIENT_WRAPPED_CEK_BYTES = 48;
const MIN_SERIALIZED_SIGNATURE_BYTES = 64;
const MAX_SERIALIZED_SIGNATURE_BYTES = 256;
const MIN_SERIALIZED_SIGNATURE_LENGTH = 80;
const MAX_SERIALIZED_SIGNATURE_LENGTH = 512;
export const DEFAULT_E2EE_CIPHER_SUITE = "xchacha20poly1305+hpke-x25519";
export const DISPUTE_SUPPLEMENTAL_BUNDLE_PROTOCOL = "clawdex.dispute-supplemental-bundle.v1";
export const CHECKPOINT_HANDOVER_PROTOCOL = "clawdex.checkpoint-handover.v1";
const SUPPLEMENTAL_EVIDENCE_CLASSES = new Set([
  "BUYER_COMPLAINT",
  "SELLER_REBUTTAL",
  "MAILBOX_COORDINATION",
  "CHECKPOINT_HANDOVER",
  "MISCONDUCT_REPORT",
  "SUPPORTING_EXHIBIT",
]);
export const KEY_AGREEMENT_FILE_VERSION = "clawnera.key-agreement.v2";
const LEGACY_KEY_AGREEMENT_FILE_VERSION = "clawnera.key-agreement.v1";
const KEY_AGREEMENT_ENVELOPE_VERSION = "clawnera.key-agreement.private-key.v1";
const KEY_AGREEMENT_ENVELOPE_CIPHER = "xchacha20poly1305";
const KEY_AGREEMENT_MASTER_KEY_FILENAME = ".key-agreement-master-key";
const KEY_AGREEMENT_MASTER_KEY_ENV = "CLAWNERA_KEY_AGREEMENT_MASTER_KEY_FILE";
const KEY_AGREEMENT_REQUIRE_EXTERNAL_MASTER_KEY_ENV = "CLAWNERA_KEY_AGREEMENT_REQUIRE_EXTERNAL_MASTER_KEY";
const KEY_AGREEMENT_AAD_PREFIX = "CLAWNERA Key Agreement Private Key v1";
const KEY_AGREEMENT_KDF_NAME = "scrypt";
const KEY_AGREEMENT_KDF_N = 32_768;
const KEY_AGREEMENT_KDF_R = 8;
const KEY_AGREEMENT_KDF_P = 1;
const KEY_AGREEMENT_KDF_MAXMEM = 64 * 1024 * 1024;
const KEY_AGREEMENT_KDF_SALT_BYTES = 32;
const KEY_AGREEMENT_MASTER_KEY_BYTES = 32;
const KEY_AGREEMENT_PRIVATE_KEY_CIPHERTEXT_MAX_BYTES = 512;
const scrypt = promisify(scryptCallback);
const CHECKPOINT_PACKET_HASH_PREFIX = "sha256:";
const CHECKPOINT_ALLOWED_ANCHOR_STATUSES = new Set(["PENDING", "CONFIRMED", "MISMATCH"]);
const CHECKPOINT_MAX_METADATA_ENTRIES = 24;
const utf8Encoder = new TextEncoder();
const PUBLIC_KEY_VALIDATION_PRIVATE_KEY = (() => {
  const seed = new Uint8Array(KEY_LENGTH);
  seed[0] = 1;
  return seed;
})();

function normalizeString(value) {
  return typeof value === "string" ? value.trim() : "";
}

function toBytes(input) {
  if (typeof input === "string") {
    return utf8Encoder.encode(input);
  }
  if (input instanceof Uint8Array) {
    return input;
  }
  if (input instanceof ArrayBuffer) {
    return new Uint8Array(input);
  }
  throw new Error("invalid_byte_input");
}

function estimateBase64UrlDecodedLength(input) {
  if (!input || !BASE64URL_NO_PAD.test(input) || input.length % 4 === 1) {
    throw new Error("invalid_base64url");
  }
  const padding = (4 - (input.length % 4)) % 4;
  return ((input.length + padding) / 4) * 3 - padding;
}

function encodeBase64Url(input) {
  return Buffer.from(input).toString("base64url");
}

function decodeBase64Url(input) {
  if (!input || !BASE64URL_NO_PAD.test(input) || input.length % 4 === 1) {
    throw new Error("invalid_base64url");
  }
  const canonicalInput = input.replace(/=+$/g, "");
  const decoded = new Uint8Array(Buffer.from(input, "base64url"));
  if (encodeBase64Url(decoded) !== canonicalInput) {
    throw new Error("invalid_base64url");
  }
  return decoded;
}

function encodeKeyMultibase(bytes, prefix = "u") {
  if (prefix === "z") {
    return `z${bs58.encode(bytes)}`;
  }
  return `u${encodeBase64Url(bytes)}`;
}

function decodeKeyMultibase(multibase) {
  const normalized = normalizeString(multibase);
  if (!normalized) {
    throw new Error("invalid_multibase_key");
  }
  if (normalized.startsWith("u")) {
    return decodeBase64Url(normalized.slice(1));
  }
  if (normalized.startsWith("z")) {
    return new Uint8Array(bs58.decode(normalized.slice(1)));
  }
  throw new Error("unsupported_multibase_prefix");
}

function assertKeyLength(bytes, kind) {
  if (!(bytes instanceof Uint8Array) || bytes.length !== KEY_LENGTH) {
    throw new Error(`invalid_${kind}_key_length`);
  }
  return bytes;
}

function isAllZero(bytes) {
  return bytes.every((byte) => byte === 0);
}

function decodeKeyAgreementPublicKey(multibase) {
  const bytes = assertKeyLength(decodeKeyMultibase(multibase), "public");
  if (isAllZero(bytes)) {
    throw new Error("invalid_public_key");
  }
  try {
    x25519.getSharedSecret(PUBLIC_KEY_VALIDATION_PRIVATE_KEY, bytes);
  } catch {
    throw new Error("invalid_public_key");
  }
  return bytes;
}

export function keyAgreementPublicKeyHex(multibase) {
  return Buffer.from(decodeKeyAgreementPublicKey(multibase)).toString("hex");
}

function decodeKeyAgreementPrivateKey(multibase) {
  const bytes = assertKeyLength(decodeKeyMultibase(multibase), "private");
  if (isAllZero(bytes)) {
    throw new Error("invalid_private_key");
  }
  return bytes;
}

function normalizeAddress(value, fieldName = "address") {
  const normalized = normalizeString(value).toLowerCase();
  if (!isValidIotaAddress(normalized)) {
    throw new Error(`invalid_${fieldName}`);
  }
  return normalizeIotaAddress(normalized);
}

function assertBoundedUtf8String(value, fieldName, maxBytes) {
  const normalized = normalizeString(value);
  if (!normalized) {
    throw new Error(`invalid_${fieldName}`);
  }
  const byteLength = utf8Encoder.encode(normalized).byteLength;
  if (byteLength === 0 || byteLength > maxBytes) {
    throw new Error(`invalid_${fieldName}`);
  }
  return normalized;
}

function assertCanonicalProtocolString(value, fieldName, maxBytes) {
  if (typeof value !== "string" || !value.trim() || value !== value.trim()) {
    throw new Error(`invalid_${fieldName}`);
  }
  const byteLength = utf8Encoder.encode(value).byteLength;
  if (byteLength === 0 || byteLength > maxBytes) {
    throw new Error(`invalid_${fieldName}`);
  }
  return value;
}

export function assertIpfsManifestCid(value) {
  const normalized = assertCanonicalProtocolString(value, "manifest_cid", MAX_MANIFEST_CID_BYTES);
  if (!/^ipfs:\/\/[a-z0-9]+(?:[/?#].*)?$/i.test(normalized)) {
    throw new Error("invalid_manifest_cid");
  }
  return normalized;
}

function normalizeNonNegativeInteger(value, fieldName) {
  const normalized = Number(value);
  if (!Number.isSafeInteger(normalized) || normalized < 0) {
    throw new Error(`invalid_${fieldName}`);
  }
  return normalized;
}

function normalizePositiveInteger(value, fieldName) {
  const normalized = Number(value);
  if (!Number.isSafeInteger(normalized) || normalized <= 0) {
    throw new Error(`invalid_${fieldName}`);
  }
  return normalized;
}

function assertUuidString(value, fieldName) {
  if (typeof value !== "string" || !UUID_PATTERN.test(value)) {
    throw new Error(`invalid_${fieldName}`);
  }
  return value;
}

function assertSupplementalEvidenceClass(value) {
  const normalized = assertCanonicalProtocolString(value, "evidence_class", 64);
  if (!SUPPLEMENTAL_EVIDENCE_CLASSES.has(normalized)) {
    throw new Error("invalid_evidence_class");
  }
  return normalized;
}

function normalizeSupplementalRecipients(recipients) {
  if (!Array.isArray(recipients) || recipients.length < SUPPLEMENTAL_RECIPIENT_MIN || recipients.length > SUPPLEMENTAL_RECIPIENT_MAX) {
    throw new Error("invalid_recipient_count");
  }
  const seenRecipientAddresses = new Set();
  return recipients.map((recipient) => {
    if (!recipient || typeof recipient !== "object" || Array.isArray(recipient)) {
      throw new Error("invalid_recipient");
    }
    const recipientAddress = normalizeAddress(recipient.recipientAddress, "recipient_address");
    if (seenRecipientAddresses.has(recipientAddress)) {
      throw new Error("duplicate_recipient_address");
    }
    seenRecipientAddresses.add(recipientAddress);
    const keyVersion = Number(recipient.keyVersion);
    if (!Number.isSafeInteger(keyVersion) || keyVersion <= 0) {
      throw new Error("invalid_recipient_key_version");
    }
    if (typeof recipient.recipientPublicKeyMultibase !== "string" || !recipient.recipientPublicKeyMultibase.trim()) {
      throw new Error("invalid_recipient_public_key");
    }
    return {
      recipientAddress,
      keyVersion,
      recipientPublicKeyMultibase: recipient.recipientPublicKeyMultibase,
    };
  });
}

export function assertLowerHex64(value, fieldName) {
  const normalized = normalizeString(value).toLowerCase();
  if (!/^[a-f0-9]{64}$/.test(normalized)) {
    throw new Error(`invalid_${fieldName}`);
  }
  return normalized;
}

function bytesToHex(value) {
  return Buffer.from(value).toString("hex");
}

function stableSortObject(value) {
  if (Array.isArray(value)) {
    return value.map((entry) => stableSortObject(entry));
  }
  if (!value || typeof value !== "object") {
    return value;
  }
  const entries = Object.entries(value).sort(([left], [right]) => left.localeCompare(right));
  return Object.fromEntries(entries.map(([key, entry]) => [key, stableSortObject(entry)]));
}

export function stableJsonStringify(value) {
  return JSON.stringify(stableSortObject(value));
}

function sha256HexBytes(value) {
  return bytesToHex(sha256(value));
}

export function sha256Hex(value) {
  return createHash("sha256").update(value).digest("hex");
}

function deriveWrapKey(sharedSecret, recipientAddress, keyVersion) {
  const info = utf8Encoder.encode(`${WRAP_INFO_PREFIX}:${recipientAddress}:${keyVersion}`);
  return hkdf(sha256, sharedSecret, undefined, info, CEK_BYTES);
}

function wrapCekForRecipient(cek, recipient) {
  const recipientAddress = normalizeAddress(recipient.recipientAddress, "recipient_address");
  const keyVersion = Number(recipient.keyVersion);
  if (!Number.isSafeInteger(keyVersion) || keyVersion <= 0) {
    throw new Error("invalid_recipient_key_version");
  }
  const recipientPublicKey = decodeKeyAgreementPublicKey(recipient.recipientPublicKeyMultibase);
  const ephemeralPrivateKey = x25519.utils.randomSecretKey();
  const ephemeralPublicKey = x25519.getPublicKey(ephemeralPrivateKey);
  const sharedSecret = x25519.getSharedSecret(ephemeralPrivateKey, recipientPublicKey);
  const wrapKey = deriveWrapKey(sharedSecret, recipientAddress, keyVersion);
  const wrapNonce = randomBytes(WRAP_NONCE_BYTES);
  const wrapCipher = xchacha20poly1305(wrapKey, wrapNonce);
  const wrappedCek = wrapCipher.encrypt(cek);
  return {
    recipientAddress,
    keyVersion,
    wrappedCek: encodeBase64Url(wrappedCek),
    hpkeEnc: `v1.${encodeBase64Url(ephemeralPublicKey)}.${encodeBase64Url(wrapNonce)}`,
  };
}

function parseHpkeEnc(hpkeEnc) {
  const parts = normalizeString(hpkeEnc).split(".");
  if (parts.length !== 3 || parts[0] !== "v1") {
    throw new Error("invalid_hpke_enc");
  }
  if (
    estimateBase64UrlDecodedLength(parts[1]) !== MANIFEST_RECIPIENT_HPKE_EPHEMERAL_BYTES ||
    estimateBase64UrlDecodedLength(parts[2]) !== MANIFEST_RECIPIENT_HPKE_NONCE_BYTES
  ) {
    throw new Error("invalid_hpke_enc");
  }
  const ephemeralPublicKey = decodeBase64Url(parts[1]);
  const wrapNonce = decodeBase64Url(parts[2]);
  if (
    ephemeralPublicKey.length !== MANIFEST_RECIPIENT_HPKE_EPHEMERAL_BYTES ||
    wrapNonce.length !== MANIFEST_RECIPIENT_HPKE_NONCE_BYTES
  ) {
    throw new Error("invalid_hpke_enc");
  }
  return { ephemeralPublicKey, wrapNonce };
}

function normalizeManifestRecipientWrappedCek(value) {
  const normalized = assertBoundedUtf8String(value, "manifest_recipient_wrapped_cek", 128);
  const decoded = decodeBase64Url(normalized);
  if (decoded.length !== MANIFEST_RECIPIENT_WRAPPED_CEK_BYTES) {
    throw new Error("invalid_manifest_recipient_wrapped_cek");
  }
  return normalized;
}

function normalizeManifestRecipientHpkeEnc(value) {
  const normalized = assertBoundedUtf8String(value, "manifest_recipient_hpke_enc", 192);
  const parts = normalized.split(".");
  if (parts.length !== 3 || parts[0] !== "v1") {
    throw new Error("invalid_manifest_recipient_hpke_enc");
  }
  const ephemeralPublicKey = decodeBase64Url(parts[1] || "");
  const wrapNonce = decodeBase64Url(parts[2] || "");
  if (
    ephemeralPublicKey.length !== MANIFEST_RECIPIENT_HPKE_EPHEMERAL_BYTES ||
    wrapNonce.length !== MANIFEST_RECIPIENT_HPKE_NONCE_BYTES
  ) {
    throw new Error("invalid_manifest_recipient_hpke_enc");
  }
  return normalized;
}

function canonicalizeJson(value) {
  if (value === null) {
    return "null";
  }
  if (typeof value === "boolean") {
    return value ? "true" : "false";
  }
  if (typeof value === "number") {
    if (!Number.isFinite(value)) {
      throw new Error("non_finite_number_not_allowed");
    }
    return JSON.stringify(value);
  }
  if (typeof value === "string") {
    return JSON.stringify(value);
  }
  if (Array.isArray(value)) {
    return `[${value.map((item) => canonicalizeJson(item)).join(",")}]`;
  }
  if (!value || typeof value !== "object") {
    throw new Error("invalid_canonical_json_value");
  }
  const keys = Object.keys(value).sort((a, b) => a.localeCompare(b));
  return `{${keys.map((key) => `${JSON.stringify(key)}:${canonicalizeJson(value[key])}`).join(",")}}`;
}

function normalizeManifestSigningContextToken(value, fieldName, maxBytes) {
  if (typeof value !== "string" || !value.trim() || value !== value.trim()) {
    throw new Error("invalid_signing_context");
  }
  const normalized = assertBoundedUtf8String(value, fieldName, maxBytes);
  if (!MANIFEST_SIGNING_CONTEXT_TOKEN.test(normalized)) {
    throw new Error("invalid_signing_context");
  }
  return normalized;
}

function createManifestNonce() {
  return randomBytes(16).reduce((out, byte) => out + byte.toString(16).padStart(2, "0"), "");
}

function normalizeManifestSigningContext(input = {}) {
  const nowMs = Date.now();
  const issuedAtMs = Number.isSafeInteger(input.issuedAtMs) ? input.issuedAtMs : nowMs;
  const ttlMs = Number.isSafeInteger(input.ttlMs) ? input.ttlMs : DEFAULT_MANIFEST_SIGNING_TTL_MS;
  const expiresAtMs = Number.isSafeInteger(input.expiresAtMs) ? input.expiresAtMs : issuedAtMs + ttlMs;
  const normalized = {
    intent: normalizeManifestSigningContextToken(
      input.intent || DEFAULT_MANIFEST_SIGNING_INTENT,
      "manifest_signing_intent",
      MAX_MANIFEST_CONTEXT_INTENT_BYTES,
    ),
    chain: normalizeManifestSigningContextToken(
      input.chain || DEFAULT_MANIFEST_SIGNING_CHAIN,
      "manifest_signing_chain",
      MAX_MANIFEST_CONTEXT_CHAIN_BYTES,
    ),
    audience: normalizeManifestSigningContextToken(
      input.audience || DEFAULT_MANIFEST_SIGNING_AUDIENCE,
      "manifest_signing_audience",
      MAX_MANIFEST_CONTEXT_AUDIENCE_BYTES,
    ),
    nonce: normalizeManifestSigningContextToken(
      input.nonce || createManifestNonce(),
      "manifest_signing_nonce",
      MAX_MANIFEST_CONTEXT_NONCE_BYTES,
    ),
    issuedAtMs,
    expiresAtMs,
  };
  if (
    !Number.isSafeInteger(normalized.issuedAtMs) ||
    !Number.isSafeInteger(normalized.expiresAtMs) ||
    normalized.issuedAtMs <= 0 ||
    normalized.expiresAtMs <= 0 ||
    normalized.issuedAtMs > normalized.expiresAtMs ||
    normalized.expiresAtMs - normalized.issuedAtMs > MAX_MANIFEST_SIGNING_WINDOW_MS
  ) {
    throw new Error("invalid_signing_context");
  }
  return normalized;
}

function buildManifestSigningIntentLine(context) {
  return [
    `intent=${context.intent}`,
    `chain=${context.chain}`,
    `aud=${context.audience}`,
    `nonce=${context.nonce}`,
    `iat=${context.issuedAtMs}`,
    `exp=${context.expiresAtMs}`,
  ].join(";");
}

function normalizeManifestRecipientsForCanonicalization(recipients) {
  if (!Array.isArray(recipients) || recipients.length === 0) {
    throw new Error("invalid_manifest_recipients");
  }
  const seen = new Set();
  return [...recipients]
    .map((recipient) => {
      const keyVersion = Number(recipient?.keyVersion);
      if (!Number.isSafeInteger(keyVersion) || keyVersion <= 0) {
        throw new Error("invalid_manifest_recipient_key_version");
      }
      const recipientAddress = normalizeAddress(recipient?.recipientAddress, "manifest_recipient_address");
      const wrappedCek = normalizeManifestRecipientWrappedCek(recipient?.wrappedCek || "");
      const hpkeEnc =
        recipient?.hpkeEnc === undefined ? undefined : normalizeManifestRecipientHpkeEnc(recipient.hpkeEnc);
      const key = `${recipientAddress}:${keyVersion}`;
      if (seen.has(key)) {
        throw new Error("duplicate_manifest_recipient");
      }
      seen.add(key);
      return { recipientAddress, keyVersion, wrappedCek, hpkeEnc };
    })
    .sort((left, right) => {
      const byAddress = left.recipientAddress.localeCompare(right.recipientAddress);
      if (byAddress !== 0) {
        return byAddress;
      }
      return left.keyVersion - right.keyVersion;
    });
}

function buildCanonicalMilestoneManifestJson(input) {
  const orderId = assertCanonicalProtocolString(input.orderId, "order_id", MAX_MANIFEST_ORDER_ID_BYTES);
  const milestoneId = assertCanonicalProtocolString(input.milestoneId, "milestone_id", MAX_MANIFEST_MILESTONE_ID_BYTES);
  const sellerAddress = normalizeAddress(input.sellerAddress, "seller_address");
  const manifestCid = assertBoundedUtf8String(input.manifestCid, "manifest_cid", MAX_MANIFEST_CID_BYTES);
  const cipherSuite = assertBoundedUtf8String(input.cipherSuite, "cipher_suite", MAX_MANIFEST_CIPHER_SUITE_BYTES);
  const recipients = normalizeManifestRecipientsForCanonicalization(input.recipients).map((recipient) => {
    const entry = {
      recipient: recipient.recipientAddress,
      keyVersion: recipient.keyVersion,
      wrappedCek: recipient.wrappedCek,
    };
    if (recipient.hpkeEnc !== undefined) {
      entry.hpkeEnc = recipient.hpkeEnc;
    }
    return entry;
  });
  const manifest = {
    manifestVersion: "v1",
    orderId,
    milestoneId,
    sellerAddress,
    blobCid: manifestCid,
    cipherSuite,
    cekWraps: recipients,
  };
  if (input.signingContext) {
    const signingContext = normalizeManifestSigningContext(input.signingContext);
    manifest.signingContext = {
      intent: signingContext.intent,
      chain: signingContext.chain,
      audience: signingContext.audience,
      nonce: signingContext.nonce,
      issuedAtMs: signingContext.issuedAtMs,
      expiresAtMs: signingContext.expiresAtMs,
    };
  }
  return canonicalizeJson(manifest);
}

function buildMilestoneManifestSigningMessage(canonicalManifestJson, signingContext) {
  if (!signingContext) {
    return `${MANIFEST_SIGNING_PREFIX}\n${LEGACY_MANIFEST_SIGNING_INTENT_LINE}\n${canonicalManifestJson}`;
  }
  return `${MANIFEST_SIGNING_PREFIX}\n${buildManifestSigningIntentLine(signingContext)}\n${canonicalManifestJson}`;
}

function formatSha256HexPrefixed(digestHex) {
  return `sha256:${digestHex.toLowerCase()}`;
}

function normalizeManifestSellerSignature(value) {
  const normalized = normalizeString(value);
  if (
    !normalized ||
    /\s/.test(normalized) ||
    normalized.length < MIN_SERIALIZED_SIGNATURE_LENGTH ||
    normalized.length > MAX_SERIALIZED_SIGNATURE_LENGTH ||
    !/^[A-Za-z0-9+/_-]+={0,2}$/.test(normalized)
  ) {
    throw new Error("invalid_seller_signature");
  }
  const decodedLength = Buffer.from(normalized, "base64").length;
  if (decodedLength < MIN_SERIALIZED_SIGNATURE_BYTES || decodedLength > MAX_SERIALIZED_SIGNATURE_BYTES) {
    throw new Error("invalid_seller_signature");
  }
  return normalized;
}

async function writeJsonFile(targetPath, payload, mode = 0o600, options = {}) {
  if (mode !== 0o600) {
    throw new Error("invalid_private_file_mode");
  }
  return writePrivateJsonAtomic(targetPath, payload, options);
}

export function defaultKeyAgreementRecordPath(address, keyVersion = 1, homeDir = os.homedir()) {
  const normalizedAddress = normalizeAddress(address);
  if (!Number.isSafeInteger(Number(keyVersion)) || Number(keyVersion) <= 0) {
    throw new Error("invalid_key_version");
  }
  return path.join(homeDir, ".config", "clawnera", "key-agreements", `${normalizedAddress}.v${Number(keyVersion)}.json`);
}

export function defaultKeyAgreementMasterKeyPath(recordPath) {
  return path.join(path.dirname(path.resolve(recordPath)), KEY_AGREEMENT_MASTER_KEY_FILENAME);
}

export function isExactKeyAgreementNotFound(result) {
  return result?.ok === false && result?.status === 404 && result?.body?.error === "key_agreement_not_found";
}

function assertExactObjectKeys(value, expectedKeys, errorCode) {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new Error(errorCode);
  }
  const actual = Object.keys(value).sort();
  const expected = [...expectedKeys].sort();
  if (actual.length !== expected.length || actual.some((entry, index) => entry !== expected[index])) {
    throw new Error(errorCode);
  }
  return value;
}

function normalizeKeyAgreementTimestamp(value, fieldName) {
  const normalized = assertBoundedUtf8String(value, fieldName, 64);
  if (!Number.isFinite(Date.parse(normalized))) {
    throw new Error(`invalid_${fieldName}`);
  }
  return normalized;
}

function keyAgreementRecordAad(metadata) {
  return toBytes(
    `${KEY_AGREEMENT_AAD_PREFIX}\n${JSON.stringify({
      version: KEY_AGREEMENT_FILE_VERSION,
      address: metadata.address,
      keyVersion: metadata.keyVersion,
      publicKeyMultibase: metadata.publicKeyMultibase,
      expiresAtMs: metadata.expiresAtMs,
      createdAt: metadata.createdAt,
      updatedAt: metadata.updatedAt,
    })}`,
  );
}

function resolveKeyAgreementMasterKeyFile(recordPath, options = {}) {
  for (const forbidden of ["passphrase", "masterSecret", "masterKey", "secret"]) {
    if (options[forbidden] !== undefined) {
      throw new Error("inline_key_agreement_secret_rejected");
    }
  }
  if (
    process.env.CLAWNERA_KEY_AGREEMENT_PASSPHRASE !== undefined ||
    process.env.CLAWNERA_KEY_AGREEMENT_MASTER_SECRET !== undefined
  ) {
    throw new Error("inline_key_agreement_secret_rejected");
  }
  const configured =
    typeof options.masterKeyFile === "string" && options.masterKeyFile.trim()
      ? options.masterKeyFile.trim()
      : typeof process.env[KEY_AGREEMENT_MASTER_KEY_ENV] === "string" && process.env[KEY_AGREEMENT_MASTER_KEY_ENV].trim()
        ? process.env[KEY_AGREEMENT_MASTER_KEY_ENV].trim()
        : "";
  const masterKeyFile = configured
    ? path.resolve(configured)
    : defaultKeyAgreementMasterKeyPath(recordPath);
  if (masterKeyFile === path.resolve(recordPath)) {
    throw new Error("key_agreement_master_key_path_conflict");
  }
  const requireExternal = process.env[KEY_AGREEMENT_REQUIRE_EXTERNAL_MASTER_KEY_ENV];
  if (requireExternal !== undefined && requireExternal !== "0" && requireExternal !== "1") {
    throw new Error("invalid_key_agreement_external_master_key_policy");
  }
  if (
    requireExternal === "1" &&
    (!configured || path.dirname(masterKeyFile) === path.dirname(path.resolve(recordPath)))
  ) {
    throw new Error("key_agreement_external_master_key_required");
  }
  return { masterKeyFile, configured: Boolean(configured) };
}

async function readKeyAgreementMasterSecret(recordPath, options = {}, allowDefaultCreate = false) {
  const { masterKeyFile, configured } = resolveKeyAgreementMasterKeyFile(recordPath, options);
  if (allowDefaultCreate && !configured) {
    const candidate = randomBytes(KEY_AGREEMENT_MASTER_KEY_BYTES);
    try {
      await writePrivateFileAtomicExclusive(masterKeyFile, candidate);
    } finally {
      candidate.fill(0);
    }
  }
  let raw;
  try {
    raw = await readPrivateFile(masterKeyFile, null);
  } catch (error) {
    if (error?.code === "ENOENT") {
      throw new Error("key_agreement_master_key_not_found");
    }
    throw error;
  }
  const secret = copyAndClearSensitiveBytes(raw);
  if (secret.length !== KEY_AGREEMENT_MASTER_KEY_BYTES) {
    secret.fill(0);
    throw new Error("invalid_key_agreement_master_key_length");
  }
  return secret;
}

async function deriveKeyAgreementEnvelopeKey(recordPath, salt, options, allowDefaultCreate) {
  const secret = await readKeyAgreementMasterSecret(recordPath, options, allowDefaultCreate);
  try {
    return new Uint8Array(
      await scrypt(secret, salt, KEY_LENGTH, {
        N: KEY_AGREEMENT_KDF_N,
        r: KEY_AGREEMENT_KDF_R,
        p: KEY_AGREEMENT_KDF_P,
        maxmem: KEY_AGREEMENT_KDF_MAXMEM,
      }),
    );
  } finally {
    secret.fill(0);
  }
}

function normalizeKeyAgreementPublicMetadata(parsed, options = {}, allowLegacyFallbacks = false) {
  const expectedAddress =
    typeof options.expectedAddress === "string" && options.expectedAddress.trim()
      ? normalizeAddress(options.expectedAddress)
      : "";
  const expectedKeyVersion =
    options.expectedKeyVersion === undefined || options.expectedKeyVersion === null
      ? undefined
      : Number(options.expectedKeyVersion);
  const fallbackExpiresAtMs =
    allowLegacyFallbacks && options.fallbackExpiresAtMs !== undefined && options.fallbackExpiresAtMs !== null
      ? Number(options.fallbackExpiresAtMs)
      : undefined;
  const address = normalizeAddress(parsed.address || (allowLegacyFallbacks ? expectedAddress : ""));
  const keyVersion = Number(parsed.keyVersion ?? (allowLegacyFallbacks ? expectedKeyVersion : undefined));
  const publicKeyMultibase = assertBoundedUtf8String(parsed.publicKeyMultibase, "public_key_multibase", 256);
  const expiresAtMs = Number(parsed.expiresAtMs ?? fallbackExpiresAtMs);
  if (!Number.isSafeInteger(keyVersion) || keyVersion <= 0 || !Number.isSafeInteger(expiresAtMs) || expiresAtMs <= 0) {
    throw new Error("invalid_key_agreement_record");
  }
  if (expectedAddress && address !== expectedAddress) {
    throw new Error("key_agreement_record_address_mismatch");
  }
  if (expectedKeyVersion !== undefined && keyVersion !== expectedKeyVersion) {
    throw new Error("key_agreement_record_version_mismatch");
  }
  return { address, keyVersion, publicKeyMultibase, expiresAtMs };
}

function assertKeyAgreementNotExpired(expiresAtMs, options = {}) {
  const nowMs = Number.isSafeInteger(options.nowMs) ? options.nowMs : Date.now();
  if (options.allowExpired !== true && expiresAtMs <= nowMs) {
    throw new Error("expired_key_agreement_record");
  }
}

function parseLegacyKeyAgreementRecord(parsed, options = {}) {
  if (parsed.version !== undefined && parsed.version !== LEGACY_KEY_AGREEMENT_FILE_VERSION) {
    throw new Error("invalid_key_agreement_record_version");
  }
  const allowedKeys = new Set([
    "version",
    "address",
    "keyVersion",
    "publicKeyMultibase",
    "privateKeyMultibase",
    "expiresAtMs",
    "createdAt",
    "updatedAt",
  ]);
  if (Object.keys(parsed).some((key) => !allowedKeys.has(key))) {
    throw new Error("invalid_key_agreement_record");
  }
  const metadata = normalizeKeyAgreementPublicMetadata(parsed, options, true);
  assertKeyAgreementNotExpired(metadata.expiresAtMs, options);
  const privateKeyMultibase = assertBoundedUtf8String(parsed.privateKeyMultibase, "private_key_multibase", 256);
  if (derivePublicKeyFromPrivateKey(privateKeyMultibase) !== metadata.publicKeyMultibase) {
    throw new Error("key_agreement_public_private_mismatch");
  }
  return {
    ...metadata,
    privateKeyMultibase,
    createdAt:
      typeof parsed.createdAt === "string"
        ? normalizeKeyAgreementTimestamp(parsed.createdAt, "created_at")
        : new Date().toISOString(),
    updatedAt:
      typeof parsed.updatedAt === "string"
        ? normalizeKeyAgreementTimestamp(parsed.updatedAt, "updated_at")
        : null,
  };
}

async function encryptKeyAgreementPrivateKey(privateKeyMultibase, metadata, recordPath, options = {}) {
  const salt = randomBytes(KEY_AGREEMENT_KDF_SALT_BYTES);
  const nonce = randomBytes(NONCE_BYTES);
  const derivedKey = await deriveKeyAgreementEnvelopeKey(recordPath, salt, options, true);
  try {
    const ciphertext = xchacha20poly1305(derivedKey, nonce, keyAgreementRecordAad(metadata)).encrypt(
      toBytes(privateKeyMultibase),
    );
    return {
      version: KEY_AGREEMENT_ENVELOPE_VERSION,
      cipher: KEY_AGREEMENT_ENVELOPE_CIPHER,
      kdf: {
        name: KEY_AGREEMENT_KDF_NAME,
        saltB64u: encodeBase64Url(salt),
        N: KEY_AGREEMENT_KDF_N,
        r: KEY_AGREEMENT_KDF_R,
        p: KEY_AGREEMENT_KDF_P,
      },
      nonceB64u: encodeBase64Url(nonce),
      ciphertextB64u: encodeBase64Url(ciphertext),
    };
  } finally {
    derivedKey.fill(0);
    salt.fill(0);
    nonce.fill(0);
  }
}

async function decryptKeyAgreementPrivateKey(envelope, metadata, recordPath, options = {}) {
  assertExactObjectKeys(
    envelope,
    ["version", "cipher", "kdf", "nonceB64u", "ciphertextB64u"],
    "invalid_key_agreement_envelope",
  );
  assertExactObjectKeys(envelope.kdf, ["name", "saltB64u", "N", "r", "p"], "invalid_key_agreement_kdf");
  if (
    envelope.version !== KEY_AGREEMENT_ENVELOPE_VERSION ||
    envelope.cipher !== KEY_AGREEMENT_ENVELOPE_CIPHER ||
    envelope.kdf.name !== KEY_AGREEMENT_KDF_NAME ||
    envelope.kdf.N !== KEY_AGREEMENT_KDF_N ||
    envelope.kdf.r !== KEY_AGREEMENT_KDF_R ||
    envelope.kdf.p !== KEY_AGREEMENT_KDF_P
  ) {
    throw new Error("invalid_key_agreement_envelope");
  }
  const salt = decodeBase64Url(envelope.kdf.saltB64u);
  const nonce = decodeBase64Url(envelope.nonceB64u);
  const ciphertext = decodeBase64Url(envelope.ciphertextB64u);
  if (
    salt.length !== KEY_AGREEMENT_KDF_SALT_BYTES ||
    nonce.length !== NONCE_BYTES ||
    ciphertext.length <= 16 ||
    ciphertext.length > KEY_AGREEMENT_PRIVATE_KEY_CIPHERTEXT_MAX_BYTES
  ) {
    throw new Error("invalid_key_agreement_envelope");
  }
  const derivedKey = await deriveKeyAgreementEnvelopeKey(recordPath, salt, options, false);
  try {
    let plaintext;
    try {
      plaintext = xchacha20poly1305(derivedKey, nonce, keyAgreementRecordAad(metadata)).decrypt(ciphertext);
      return new TextDecoder("utf-8", { fatal: true }).decode(plaintext);
    } catch {
      throw new Error("key_agreement_record_decryption_failed");
    } finally {
      plaintext?.fill(0);
    }
  } finally {
    derivedKey.fill(0);
    salt.fill(0);
    nonce.fill(0);
    ciphertext.fill(0);
  }
}

export async function loadKeyAgreementRecord(filePath, options = {}) {
  const resolved = path.resolve(filePath);
  const raw = await readPrivateFile(resolved, "utf8");
  let parsed;
  try {
    parsed = JSON.parse(raw);
  } catch {
    throw new Error("invalid_key_agreement_record_json");
  }
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
    throw new Error("invalid_key_agreement_record");
  }
  if (Object.prototype.hasOwnProperty.call(parsed, "privateKeyMultibase")) {
    throw new Error("plaintext_key_agreement_record_rejected");
  }
  assertExactObjectKeys(
    parsed,
    [
      "version",
      "address",
      "keyVersion",
      "publicKeyMultibase",
      "expiresAtMs",
      "createdAt",
      "updatedAt",
      "privateKeyEnvelope",
    ],
    "invalid_key_agreement_record",
  );
  if (parsed.version !== KEY_AGREEMENT_FILE_VERSION) {
    throw new Error("invalid_key_agreement_record_version");
  }
  const metadata = {
    ...normalizeKeyAgreementPublicMetadata(parsed, options, false),
    createdAt: normalizeKeyAgreementTimestamp(parsed.createdAt, "created_at"),
    updatedAt: normalizeKeyAgreementTimestamp(parsed.updatedAt, "updated_at"),
  };
  assertKeyAgreementNotExpired(metadata.expiresAtMs, options);
  const privateKeyMultibase = assertBoundedUtf8String(
    await decryptKeyAgreementPrivateKey(parsed.privateKeyEnvelope, metadata, resolved, options),
    "private_key_multibase",
    256,
  );
  if (derivePublicKeyFromPrivateKey(privateKeyMultibase) !== metadata.publicKeyMultibase) {
    throw new Error("key_agreement_public_private_mismatch");
  }
  return {
    version: KEY_AGREEMENT_FILE_VERSION,
    ...metadata,
    privateKeyMultibase,
    filePath: resolved,
  };
}

async function saveKeyAgreementRecordInternal(input, { allowExpired = false } = {}) {
  const address = normalizeAddress(input.address);
  const keyVersion = Number(input.keyVersion);
  const publicKeyMultibase = assertBoundedUtf8String(input.publicKeyMultibase, "public_key_multibase", 256);
  const privateKeyMultibase = assertBoundedUtf8String(input.privateKeyMultibase, "private_key_multibase", 256);
  const expiresAtMs = Number(input.expiresAtMs);
  if (!Number.isSafeInteger(keyVersion) || keyVersion <= 0 || !Number.isSafeInteger(expiresAtMs) || expiresAtMs <= 0) {
    throw new Error("invalid_key_agreement_record");
  }
  if (!allowExpired && expiresAtMs <= Date.now()) {
    throw new Error("expired_key_agreement_record");
  }
  if (derivePublicKeyFromPrivateKey(privateKeyMultibase) !== publicKeyMultibase) {
    throw new Error("key_agreement_public_private_mismatch");
  }
  const createdAt =
    typeof input.createdAt === "string"
      ? normalizeKeyAgreementTimestamp(input.createdAt, "created_at")
      : new Date().toISOString();
  const updatedAt = new Date().toISOString();
  const filePath =
    typeof input.filePath === "string" && input.filePath.trim()
      ? path.resolve(input.filePath)
      : defaultKeyAgreementRecordPath(address, keyVersion);
  const metadata = { address, keyVersion, publicKeyMultibase, expiresAtMs, createdAt, updatedAt };
  const privateKeyEnvelope = await encryptKeyAgreementPrivateKey(privateKeyMultibase, metadata, filePath, input);
  const payload = {
    version: KEY_AGREEMENT_FILE_VERSION,
    ...metadata,
    privateKeyEnvelope,
  };
  const writeOptions = Object.prototype.hasOwnProperty.call(input, "expectedRecordSha256")
    ? { expectedSha256: input.expectedRecordSha256 }
    : {};
  const savedPath = await writeJsonFile(filePath, payload, 0o600, writeOptions);
  return {
    version: KEY_AGREEMENT_FILE_VERSION,
    ...metadata,
    privateKeyMultibase,
    filePath: savedPath,
  };
}

export async function saveKeyAgreementRecord(input) {
  return saveKeyAgreementRecordInternal(input);
}

export async function migrateLegacyKeyAgreementRecord(filePath, options = {}) {
  const resolved = path.resolve(filePath);
  const raw = await readPrivateFile(resolved, "utf8");
  const expectedRecordSha256 = createHash("sha256").update(raw).digest("hex");
  let parsed;
  try {
    parsed = JSON.parse(raw);
  } catch {
    throw new Error("invalid_key_agreement_record_json");
  }
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
    throw new Error("invalid_key_agreement_record");
  }
  if (!Object.prototype.hasOwnProperty.call(parsed, "privateKeyMultibase")) {
    if (parsed.version === KEY_AGREEMENT_FILE_VERSION) {
      throw new Error("key_agreement_record_already_encrypted");
    }
    throw new Error("plaintext_key_agreement_record_not_found");
  }
  const legacy = parseLegacyKeyAgreementRecord(parsed, { ...options, allowExpired: true });
  const migrated = await saveKeyAgreementRecordInternal(
    {
      ...legacy,
      filePath: resolved,
      masterKeyFile: options.masterKeyFile,
      expectedRecordSha256,
    },
    { allowExpired: true },
  );
  return {
    version: migrated.version,
    address: migrated.address,
    keyVersion: migrated.keyVersion,
    publicKeyMultibase: migrated.publicKeyMultibase,
    expiresAtMs: migrated.expiresAtMs,
    createdAt: migrated.createdAt,
    updatedAt: migrated.updatedAt,
    expired: migrated.expiresAtMs <= (Number.isSafeInteger(options.nowMs) ? options.nowMs : Date.now()),
    filePath: migrated.filePath,
  };
}

export function generateKeyAgreementKeypair(prefix = "u") {
  const privateKey = x25519.utils.randomSecretKey();
  const publicKey = x25519.getPublicKey(privateKey);
  return {
    privateKeyMultibase: encodeKeyMultibase(privateKey, prefix),
    publicKeyMultibase: encodeKeyMultibase(publicKey, prefix),
  };
}

export function derivePublicKeyFromPrivateKey(multibasePrivateKey, prefix = "u") {
  const privateKey = decodeKeyAgreementPrivateKey(multibasePrivateKey);
  return encodeKeyMultibase(x25519.getPublicKey(privateKey), prefix);
}

export function buildKeyAgreementBindingMessage(address, keyVersion, publicKeyMultibase, expiresAtMs) {
  const normalizedAddress = normalizeAddress(address);
  const normalizedKeyVersion = Number(keyVersion);
  if (!Number.isSafeInteger(normalizedKeyVersion) || normalizedKeyVersion <= 0) {
    throw new Error("invalid_key_version");
  }
  const normalizedPublicKey = assertBoundedUtf8String(publicKeyMultibase, "public_key_multibase", 256);
  if (!Number.isSafeInteger(Number(expiresAtMs)) || Number(expiresAtMs) <= 0) {
    throw new Error("invalid_expires_at_ms");
  }
  return [
    "CLAWDEX Key Agreement Binding",
    `address:${normalizedAddress}`,
    `keyVersion:${normalizedKeyVersion}`,
    `keyAgreementPublicKey:${normalizedPublicKey}`,
    `expiresAtMs:${Number(expiresAtMs)}`,
  ].join("\n");
}

export async function createEncryptedDeliverable(input) {
  if (!Array.isArray(input?.recipients) || input.recipients.length === 0) {
    throw new Error("recipients_required");
  }
  const plaintext = toBytes(input.plaintext);
  if (plaintext.length > MAX_DELIVERABLE_PLAINTEXT_BYTES) {
    throw new Error("deliverable_plaintext_too_large");
  }
  const aad = input.aad === undefined ? undefined : toBytes(input.aad);

  const cek = randomBytes(CEK_BYTES);
  const blobNonce = randomBytes(NONCE_BYTES);
  const blobCipher = xchacha20poly1305(cek, blobNonce, aad);
  const ciphertext = blobCipher.encrypt(plaintext);
  const ciphertextSha256 = bytesToHex(sha256(ciphertext));

  const cekWraps = input.recipients.map((recipient) => {
    const recipientAddress = normalizeAddress(recipient.recipientAddress, "recipient_address");
    const keyVersion = Number(recipient.keyVersion);
    if (!Number.isSafeInteger(keyVersion) || keyVersion <= 0) {
      throw new Error("invalid_recipient_key_version");
    }
    const recipientPublicKey = decodeKeyAgreementPublicKey(recipient.recipientPublicKeyMultibase);
    const ephemeralPrivateKey = x25519.utils.randomSecretKey();
    const ephemeralPublicKey = x25519.getPublicKey(ephemeralPrivateKey);
    const sharedSecret = x25519.getSharedSecret(ephemeralPrivateKey, recipientPublicKey);
    const wrapKey = deriveWrapKey(sharedSecret, recipientAddress, keyVersion);
    const wrapNonce = randomBytes(WRAP_NONCE_BYTES);
    const wrapCipher = xchacha20poly1305(wrapKey, wrapNonce);
    const wrappedCek = wrapCipher.encrypt(cek);
    return {
      recipientAddress,
      keyVersion,
      wrappedCek: encodeBase64Url(wrappedCek),
      hpkeEnc: `v1.${encodeBase64Url(ephemeralPublicKey)}.${encodeBase64Url(wrapNonce)}`,
    };
  });

  return {
    cipherSuite: DEFAULT_E2EE_CIPHER_SUITE,
    blob: {
      nonceB64u: encodeBase64Url(blobNonce),
      ciphertextB64u: encodeBase64Url(ciphertext),
      plaintextByteLength: plaintext.length,
      ciphertextByteLength: ciphertext.length,
      ciphertextSha256,
    },
    cekWraps,
  };
}

export async function decryptDeliverableForRecipient(input) {
  if (
    !Number.isSafeInteger(input?.blob?.plaintextByteLength) ||
    input.blob.plaintextByteLength < 0 ||
    input.blob.plaintextByteLength > MAX_DELIVERABLE_PLAINTEXT_BYTES
  ) {
    throw new Error("invalid_plaintext_length");
  }
  if (
    !Number.isSafeInteger(input?.blob?.ciphertextByteLength) ||
    input.blob.ciphertextByteLength <= 0 ||
    input.blob.ciphertextByteLength > MAX_DELIVERABLE_CIPHERTEXT_BYTES ||
    input.blob.ciphertextByteLength !== input.blob.plaintextByteLength + 16
  ) {
    throw new Error("invalid_ciphertext_length");
  }
  if (estimateBase64UrlDecodedLength(input.wrap.wrappedCek) !== WRAPPED_CEK_BYTES) {
    throw new Error("invalid_wrapped_cek");
  }
  if (estimateBase64UrlDecodedLength(input.blob.nonceB64u) !== NONCE_BYTES) {
    throw new Error("invalid_blob_nonce");
  }
  if (estimateBase64UrlDecodedLength(input.blob.ciphertextB64u) !== input.blob.ciphertextByteLength) {
    throw new Error("invalid_ciphertext_length");
  }
  const recipientPrivateKey = decodeKeyAgreementPrivateKey(input.recipientPrivateKeyMultibase);
  const { ephemeralPublicKey, wrapNonce } = parseHpkeEnc(input.wrap.hpkeEnc);
  const sharedSecret = x25519.getSharedSecret(recipientPrivateKey, ephemeralPublicKey);
  const wrapKey = deriveWrapKey(sharedSecret, normalizeAddress(input.wrap.recipientAddress), Number(input.wrap.keyVersion));
  const wrappedCek = decodeBase64Url(input.wrap.wrappedCek);
  const wrapCipher = xchacha20poly1305(wrapKey, wrapNonce);
  const cek = wrapCipher.decrypt(wrappedCek);
  if (cek.length !== CEK_BYTES) {
    throw new Error("invalid_cek_length");
  }
  const blobNonce = decodeBase64Url(input.blob.nonceB64u);
  if (blobNonce.length !== NONCE_BYTES) {
    throw new Error("invalid_blob_nonce");
  }
  const ciphertext = decodeBase64Url(input.blob.ciphertextB64u);
  if (ciphertext.length !== input.blob.ciphertextByteLength) {
    throw new Error("invalid_ciphertext_length");
  }
  if (sha256HexBytes(ciphertext) !== normalizeString(input.blob.ciphertextSha256).toLowerCase()) {
    throw new Error("invalid_ciphertext_sha256");
  }
  const aad = input.aad === undefined ? undefined : toBytes(input.aad);
  const blobCipher = xchacha20poly1305(cek, blobNonce, aad);
  const plaintext = blobCipher.decrypt(ciphertext);
  if (plaintext.length !== input.blob.plaintextByteLength) {
    throw new Error("invalid_plaintext_length");
  }
  return plaintext;
}

export async function rewrapManagedDeliverableForRecipients(input) {
  const actorAddress = normalizeAddress(input?.actorAddress, "actor_address");
  const payload = normalizeManagedDeliverablePayload(input?.payload);
  if (!Array.isArray(input?.newRecipients) || input.newRecipients.length === 0) {
    throw new Error("recipients_required");
  }
  const existingWrap = payload.encrypted.cekWraps.find((entry) => normalizeAddress(entry.recipientAddress) === actorAddress);
  if (!existingWrap) {
    throw new Error("recipient_wrap_not_found");
  }
  const recipientPrivateKey = decodeKeyAgreementPrivateKey(input.recipientPrivateKeyMultibase);
  const { ephemeralPublicKey, wrapNonce } = parseHpkeEnc(existingWrap.hpkeEnc);
  const sharedSecret = x25519.getSharedSecret(recipientPrivateKey, ephemeralPublicKey);
  const wrapKey = deriveWrapKey(sharedSecret, normalizeAddress(existingWrap.recipientAddress), Number(existingWrap.keyVersion));
  const wrappedCek = decodeBase64Url(existingWrap.wrappedCek);
  const wrapCipher = xchacha20poly1305(wrapKey, wrapNonce);
  const cek = wrapCipher.decrypt(wrappedCek);
  if (cek.length !== CEK_BYTES) {
    throw new Error("invalid_cek_length");
  }
  return {
    cekWraps: input.newRecipients.map((recipient) => {
      const recipientAddress = normalizeAddress(recipient.recipientAddress, "recipient_address");
      const keyVersion = Number(recipient.keyVersion);
      if (!Number.isSafeInteger(keyVersion) || keyVersion <= 0) {
        throw new Error("invalid_recipient_key_version");
      }
      return wrapCekForRecipient(cek, {
        recipientAddress,
        keyVersion,
        recipientPublicKeyMultibase: recipient.recipientPublicKeyMultibase,
      });
    }),
  };
}

export async function prepareMilestoneManifestForSigning(input) {
  const orderId = assertCanonicalProtocolString(input.orderId, "order_id", MAX_MANIFEST_ORDER_ID_BYTES);
  const milestoneId = assertCanonicalProtocolString(input.milestoneId, "milestone_id", MAX_MANIFEST_MILESTONE_ID_BYTES);
  const sellerAddress = normalizeAddress(input.sellerAddress, "seller_address");
  const sellerKeyVersion = Number(input.sellerKeyVersion);
  if (!Number.isSafeInteger(sellerKeyVersion) || sellerKeyVersion <= 0) {
    throw new Error("invalid_seller_key_version");
  }
  const manifestCid = assertBoundedUtf8String(input.manifestCid, "manifest_cid", MAX_MANIFEST_CID_BYTES);
  const cipherSuite = assertBoundedUtf8String(input.cipherSuite, "cipher_suite", MAX_MANIFEST_CIPHER_SUITE_BYTES);
  const recipients = normalizeManifestRecipientsForCanonicalization(input.recipients);
  const signingContext = normalizeManifestSigningContext(input.signingContext);
  const canonicalManifestJson = buildCanonicalMilestoneManifestJson({
    orderId,
    milestoneId,
    sellerAddress,
    manifestCid,
    cipherSuite,
    recipients,
    signingContext,
  });
  const manifestSha256 = await sha256Hex(canonicalManifestJson);
  return {
    canonicalManifestJson,
    manifestSha256,
    signingMessage: buildMilestoneManifestSigningMessage(canonicalManifestJson, signingContext),
    submissionProofHash: formatSha256HexPrefixed(manifestSha256),
    submissionRef: manifestCid,
    manifest: {
      manifestVersion: "v1",
      manifestCid,
      manifestSha256,
      sellerKeyVersion,
      cipherSuite,
      signingContext,
      recipients,
    },
  };
}

export function buildSignedMilestoneSubmitPayload(prepared, sellerSignature) {
  return {
    submissionProofHash: prepared.submissionProofHash,
    submissionRef: prepared.submissionRef,
    manifest: {
      ...prepared.manifest,
      sellerSignature: normalizeManifestSellerSignature(sellerSignature),
    },
  };
}

export function buildManagedDeliverablePayload({
  orderId,
  milestoneId,
  plaintextLabel,
  encrypted,
  generatedAt = new Date().toISOString(),
}) {
  return {
    protocol: "clawdex.managed-deliverable.v1",
    orderId: assertCanonicalProtocolString(orderId, "order_id", MAX_MANIFEST_ORDER_ID_BYTES),
    milestoneId: assertCanonicalProtocolString(milestoneId, "milestone_id", MAX_MANIFEST_MILESTONE_ID_BYTES),
    generatedAt,
    metadata: {
      plaintextLabel: normalizeString(plaintextLabel) || "deliverable",
    },
    encrypted: {
      blob: {
        nonceB64u: encrypted.blob.nonceB64u,
        ciphertextB64u: encrypted.blob.ciphertextB64u,
        plaintextByteLength: encrypted.blob.plaintextByteLength,
        ciphertextByteLength: encrypted.blob.ciphertextByteLength,
        ciphertextSha256: encrypted.blob.ciphertextSha256,
      },
      cekWraps: encrypted.cekWraps.map((entry) => ({
        recipientAddress: normalizeAddress(entry.recipientAddress),
        keyVersion: Number(entry.keyVersion),
        wrappedCek: entry.wrappedCek,
        hpkeEnc: entry.hpkeEnc,
      })),
    },
  };
}

export function deriveDisputeSupplementalSummary(bundlePlaintext) {
  const record =
    bundlePlaintext && typeof bundlePlaintext === "object" && !Array.isArray(bundlePlaintext) ? bundlePlaintext : {};
  const statement =
    record.statement && typeof record.statement === "object" && !Array.isArray(record.statement)
      ? record.statement
      : null;
  const items = Array.isArray(record.items) ? record.items : [];
  let attachmentCount = 0;
  let mailboxSignalCount = 0;
  let mailboxAckCount = 0;
  let checkpointRefCount = 0;
  for (const item of items) {
    if (!item || typeof item !== "object" || Array.isArray(item)) {
      continue;
    }
    const itemType = normalizeString(item.itemType).toLowerCase();
    if (!itemType) {
      attachmentCount += 1;
      continue;
    }
    if (itemType === "mailbox_signal_ref") {
      mailboxSignalCount += 1;
      continue;
    }
    if (itemType === "mailbox_ack_ref") {
      mailboxAckCount += 1;
      continue;
    }
    if (itemType === "checkpoint_packet" || itemType === "checkpoint_ref") {
      checkpointRefCount += 1;
      continue;
    }
    attachmentCount += 1;
  }
  return {
    containsStatement: Boolean(statement),
    attachmentCount,
    mailboxSignalCount,
    mailboxAckCount,
    checkpointRefCount,
  };
}

function canonicalizeCheckpointPacketJson(value) {
  if (value === null) {
    return "null";
  }
  if (typeof value === "boolean") {
    return value ? "true" : "false";
  }
  if (typeof value === "number") {
    if (!Number.isFinite(value)) {
      throw new Error("invalid_checkpoint_packet_number");
    }
    return JSON.stringify(value);
  }
  if (typeof value === "string") {
    return JSON.stringify(value);
  }
  if (Array.isArray(value)) {
    return `[${value.map((entry) => canonicalizeCheckpointPacketJson(entry)).join(",")}]`;
  }
  const entries = Object.entries(value).sort(([left], [right]) => left.localeCompare(right));
  return `{${entries.map(([key, entry]) => `${JSON.stringify(key)}:${canonicalizeCheckpointPacketJson(entry)}`).join(",")}}`;
}

function normalizeCheckpointSimpleId(value, fieldName) {
  const normalized = normalizeString(value);
  if (!/^[A-Za-z0-9._:-]{1,200}$/.test(normalized)) {
    throw new Error(`invalid_${fieldName}`);
  }
  return normalized;
}

function normalizeCheckpointPlainString(value, fieldName, maxBytes) {
  const normalized = normalizeString(value);
  if (!normalized || /[\r\n]/.test(normalized) || utf8Encoder.encode(normalized).byteLength > maxBytes) {
    throw new Error(`invalid_${fieldName}`);
  }
  return normalized;
}

function normalizeCheckpointPositiveInteger(value, fieldName) {
  const parsed =
    typeof value === "number" && Number.isFinite(value)
      ? Number.parseInt(String(value), 10)
      : typeof value === "string" && /^[0-9]+$/.test(value.trim())
        ? Number.parseInt(value.trim(), 10)
        : NaN;
  if (!Number.isSafeInteger(parsed) || parsed <= 0) {
    throw new Error(`invalid_${fieldName}`);
  }
  return parsed;
}

function normalizeCheckpointAnchor(anchor) {
  if (anchor === undefined || anchor === null) {
    return undefined;
  }
  if (typeof anchor !== "object" || Array.isArray(anchor)) {
    throw new Error("invalid_anchor");
  }
  const txDigest = normalizeCheckpointPlainString(anchor.txDigest, "anchor_tx_digest", 128);
  if (!isValidTransactionDigest(txDigest)) {
    throw new Error("invalid_anchor_tx_digest");
  }
  const eventSeq =
    anchor.eventSeq === undefined || anchor.eventSeq === null
      ? undefined
      : normalizeCheckpointPlainString(String(anchor.eventSeq), "anchor_event_seq", 32);
  const status =
    anchor.status === undefined || anchor.status === null
      ? undefined
      : normalizeCheckpointPlainString(String(anchor.status).toUpperCase(), "anchor_status", 16);
  if (status && !CHECKPOINT_ALLOWED_ANCHOR_STATUSES.has(status)) {
    throw new Error("invalid_anchor_status");
  }
  const anchoredAtMs =
    anchor.anchoredAtMs === undefined || anchor.anchoredAtMs === null
      ? undefined
      : normalizeCheckpointPositiveInteger(anchor.anchoredAtMs, "anchor_anchored_at_ms");
  return {
    txDigest,
    ...(eventSeq ? { eventSeq } : {}),
    ...(status ? { status } : {}),
    ...(anchoredAtMs !== undefined ? { anchoredAtMs } : {}),
  };
}

function normalizeCheckpointMetadata(metadata) {
  if (metadata === undefined || metadata === null) {
    return undefined;
  }
  if (typeof metadata !== "object" || Array.isArray(metadata)) {
    throw new Error("invalid_checkpoint_metadata");
  }
  const entries = Object.entries(metadata);
  if (entries.length === 0 || entries.length > CHECKPOINT_MAX_METADATA_ENTRIES) {
    throw new Error("invalid_checkpoint_metadata");
  }
  const normalized = Object.fromEntries(
    entries.map(([key, value]) => {
      if (!/^[A-Za-z0-9._:-]{1,40}$/.test(key)) {
        throw new Error("invalid_checkpoint_metadata_key");
      }
      if (typeof value !== "string") {
        throw new Error("invalid_checkpoint_metadata_value");
      }
      return [key, normalizeCheckpointPlainString(value, "checkpoint_metadata_value", 240)];
    }),
  );
  return Object.fromEntries(Object.entries(normalized).sort(([left], [right]) => left.localeCompare(right)));
}

export function buildCheckpointHandoverPacket({
  orderId,
  milestoneId,
  checkpointId,
  stageIndex = 1,
  stageTotal = 1,
  senderRole,
  createdAtMs = Date.now(),
  deliverable,
  anchor,
  metadata,
}) {
  const normalizedSenderRole = normalizeString(senderRole).toLowerCase();
  if (!["buyer", "seller"].includes(normalizedSenderRole)) {
    throw new Error("invalid_sender_role");
  }
  const normalizedStageIndex = normalizeCheckpointPositiveInteger(stageIndex, "stage_index");
  const normalizedStageTotal = normalizeCheckpointPositiveInteger(stageTotal, "stage_total");
  if (normalizedStageIndex > normalizedStageTotal || normalizedStageTotal > 100) {
    throw new Error("invalid_stage_range");
  }
  if (!deliverable || typeof deliverable !== "object" || Array.isArray(deliverable)) {
    throw new Error("invalid_deliverable");
  }
  const packetWithoutHash = {
    protocol: CHECKPOINT_HANDOVER_PROTOCOL,
    orderId: normalizeCheckpointSimpleId(orderId, "order_id"),
    milestoneId: normalizeCheckpointSimpleId(milestoneId, "milestone_id"),
    checkpointId: normalizeCheckpointSimpleId(checkpointId, "checkpoint_id"),
    stageIndex: normalizedStageIndex,
    stageTotal: normalizedStageTotal,
    senderRole: normalizedSenderRole,
    createdAtMs: normalizeCheckpointPositiveInteger(createdAtMs, "created_at_ms"),
    deliverable: {
      cipherSuite: normalizeCheckpointPlainString(deliverable.cipherSuite, "cipher_suite", 120),
      manifestCid: assertIpfsManifestCid(deliverable.manifestCid),
      manifestSha256: assertLowerHex64(deliverable.manifestSha256, "manifest_sha256"),
      ciphertextSha256: assertLowerHex64(deliverable.ciphertextSha256, "ciphertext_sha256"),
    },
    ...(anchor ? { anchor: normalizeCheckpointAnchor(anchor) } : {}),
    ...(metadata ? { metadata: normalizeCheckpointMetadata(metadata) } : {}),
  };
  return {
    ...packetWithoutHash,
    packetHash: `${CHECKPOINT_PACKET_HASH_PREFIX}${sha256Hex(canonicalizeCheckpointPacketJson(packetWithoutHash))}`,
  };
}

export async function buildDisputeSupplementalBundlePayload({
  disputeCaseObjectId,
  orderId,
  milestoneId,
  assignmentRound,
  evidenceClass,
  declaredByActorAddress,
  plaintextBundle,
  recipients,
  replyToEvidenceId,
  generatedAt = new Date().toISOString(),
}) {
  const normalizedPlaintext =
    plaintextBundle && typeof plaintextBundle === "object" && !Array.isArray(plaintextBundle)
      ? stableSortObject(plaintextBundle)
      : null;
  if (!normalizedPlaintext || Object.keys(normalizedPlaintext).length === 0) {
    throw new Error("invalid_supplemental_bundle_plaintext");
  }
  const canonicalPlaintextJson = stableJsonStringify(normalizedPlaintext);
  const normalizedRecipients = normalizeSupplementalRecipients(recipients);
  const encrypted = await createEncryptedDeliverable({
    plaintext: canonicalPlaintextJson,
    recipients: normalizedRecipients,
  });
  const payload = {
    protocol: DISPUTE_SUPPLEMENTAL_BUNDLE_PROTOCOL,
    disputeCaseObjectId: normalizeAddress(disputeCaseObjectId, "dispute_case_object_id"),
    orderId: assertCanonicalProtocolString(orderId, "order_id", MAX_MANIFEST_ORDER_ID_BYTES),
    milestoneId: assertCanonicalProtocolString(milestoneId, "milestone_id", MAX_MANIFEST_MILESTONE_ID_BYTES),
    assignmentRound: normalizeNonNegativeInteger(assignmentRound, "assignment_round"),
    evidenceClass: assertSupplementalEvidenceClass(evidenceClass),
    declaredByActorAddress: normalizeAddress(declaredByActorAddress, "declared_by_actor_address"),
    ...(replyToEvidenceId ? { replyToEvidenceId: assertUuidString(replyToEvidenceId, "reply_to_evidence_id") } : {}),
    generatedAt: typeof generatedAt === "string" && generatedAt.trim() ? generatedAt : new Date().toISOString(),
    encrypted: {
      blob: {
        nonceB64u: encrypted.blob.nonceB64u,
        ciphertextB64u: encrypted.blob.ciphertextB64u,
        plaintextByteLength: encrypted.blob.plaintextByteLength,
        ciphertextByteLength: encrypted.blob.ciphertextByteLength,
        ciphertextSha256: encrypted.blob.ciphertextSha256,
      },
      cekWraps: encrypted.cekWraps.map((entry) => ({
        recipientAddress: normalizeAddress(entry.recipientAddress),
        keyVersion: Number(entry.keyVersion),
        wrappedCek: entry.wrappedCek,
        hpkeEnc: entry.hpkeEnc,
      })),
    },
  };
  return {
    payload,
    summary: deriveDisputeSupplementalSummary(normalizedPlaintext),
    canonicalPayloadJson: stableJsonStringify(payload),
    canonicalPlaintextJson,
    cipherSuite: encrypted.cipherSuite,
  };
}

export function normalizeDisputeSupplementalBundlePayload(payload) {
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) {
    throw new Error("invalid_supplemental_bundle_payload");
  }
  if (payload.protocol !== DISPUTE_SUPPLEMENTAL_BUNDLE_PROTOCOL) {
    throw new Error("invalid_supplemental_bundle_protocol");
  }
  const encrypted = payload.encrypted;
  if (!encrypted || typeof encrypted !== "object" || Array.isArray(encrypted)) {
    throw new Error("invalid_supplemental_bundle_payload");
  }
  return {
    protocol: DISPUTE_SUPPLEMENTAL_BUNDLE_PROTOCOL,
    disputeCaseObjectId: normalizeAddress(payload.disputeCaseObjectId, "dispute_case_object_id"),
    orderId: assertCanonicalProtocolString(payload.orderId, "order_id", MAX_MANIFEST_ORDER_ID_BYTES),
    milestoneId: assertCanonicalProtocolString(payload.milestoneId, "milestone_id", MAX_MANIFEST_MILESTONE_ID_BYTES),
    assignmentRound: normalizeNonNegativeInteger(payload.assignmentRound, "assignment_round"),
    evidenceClass: assertSupplementalEvidenceClass(payload.evidenceClass),
    declaredByActorAddress: normalizeAddress(payload.declaredByActorAddress, "declared_by_actor_address"),
    ...(payload.replyToEvidenceId
      ? { replyToEvidenceId: assertUuidString(payload.replyToEvidenceId, "reply_to_evidence_id") }
      : {}),
    generatedAt: typeof payload.generatedAt === "string" ? payload.generatedAt : new Date().toISOString(),
    encrypted: {
      blob: {
        nonceB64u: encrypted.blob?.nonceB64u,
        ciphertextB64u: encrypted.blob?.ciphertextB64u,
        plaintextByteLength: Number(encrypted.blob?.plaintextByteLength),
        ciphertextByteLength: Number(encrypted.blob?.ciphertextByteLength),
        ciphertextSha256: encrypted.blob?.ciphertextSha256,
      },
      cekWraps: Array.isArray(encrypted.cekWraps)
        ? encrypted.cekWraps.map((entry) => ({
            recipientAddress: normalizeAddress(entry.recipientAddress, "recipient_address"),
            keyVersion: normalizePositiveInteger(entry.keyVersion, "recipient_key_version"),
            wrappedCek: assertCanonicalProtocolString(entry.wrappedCek, "wrapped_cek", 8_192),
            hpkeEnc: assertCanonicalProtocolString(entry.hpkeEnc, "hpke_enc", 8_192),
          }))
        : [],
    },
  };
}

export async function decryptDisputeSupplementalBundleForRecipient({
  payload,
  wrap,
  recipientPrivateKeyMultibase,
}) {
  const normalizedPayload = normalizeDisputeSupplementalBundlePayload(payload);
  const plaintext = await decryptDeliverableForRecipient({
    blob: normalizedPayload.encrypted.blob,
    wrap,
    recipientPrivateKeyMultibase,
  });
  const plaintextText = Buffer.from(plaintext).toString("utf8");
  let plaintextJson;
  try {
    plaintextJson = JSON.parse(plaintextText);
  } catch {
    throw new Error("invalid_supplemental_bundle_plaintext_json");
  }
  return {
    plaintext,
    plaintextJson,
    plaintextSha256: sha256Hex(Buffer.from(plaintext)),
  };
}

export function normalizeManagedDeliverablePayload(payload) {
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) {
    throw new Error("invalid_deliverable_payload");
  }
  const encrypted = payload.encrypted;
  if (!encrypted || typeof encrypted !== "object" || Array.isArray(encrypted)) {
    throw new Error("invalid_deliverable_payload");
  }
  return buildManagedDeliverablePayload({
    orderId: payload.orderId,
    milestoneId: payload.milestoneId,
    plaintextLabel: payload.metadata?.plaintextLabel || "deliverable",
    encrypted: {
      blob: {
        nonceB64u: encrypted.blob?.nonceB64u,
        ciphertextB64u: encrypted.blob?.ciphertextB64u,
        plaintextByteLength: Number(encrypted.blob?.plaintextByteLength),
        ciphertextByteLength: Number(encrypted.blob?.ciphertextByteLength),
        ciphertextSha256: encrypted.blob?.ciphertextSha256,
      },
      cekWraps: Array.isArray(encrypted.cekWraps) ? encrypted.cekWraps : [],
    },
    generatedAt: typeof payload.generatedAt === "string" ? payload.generatedAt : new Date().toISOString(),
  });
}

export async function writeManagedDeliverablePayload(filePath, payload) {
  return await writeJsonFile(filePath, normalizeManagedDeliverablePayload(payload));
}
