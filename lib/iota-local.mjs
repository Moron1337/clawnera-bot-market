...
export const IOTA_NETWORK_ENV_NAMES = Object.freeze(["CLAWNERA_IOTA_NETWORK", "IOTA_NETWORK"]);
export const IOTA_RPC_URL_ENV_NAMES = Object.freeze(["CLAWNERA_IOTA_RPC_URL", "IOTA_RPC_URL"]);
export const IOTA_FAUCET_URL_ENV_NAMES = Object.freeze(["CLAWNERA_IOTA_FAUCET_URL", "IOTA_FAUCET_URL"]);
...
function normalizeRpcUrl(value) {
  ...
  try {
    return normalizeAuthenticatedUrl(normalized, { errorCode: "invalid_iota_rpc_url" });
  } catch {
    throw new Error("invalid_iota_rpc_url");
  }
}

function firstNonEmptyEnvValue(names, env = process.env) {
...
function resolveRequestedNetwork(options = {}, env = process.env) {
  return normalizeString(options.network) || firstNonEmptyEnvValue(IOTA_NETWORK_ENV_NAMES, env) || DEFAULT_IOTA_NETWORK;
}

function resolveRequestedRpcUrl(options = {}, env = process.env) {
  return normalizeString(options.rpcUrl) || firstNonEmptyEnvValue(IOTA_RPC_URL_ENV_NAMES, env);
}

function normalizeFaucetUrl(value) {
  const normalized = normalizeString(value);
  if (!normalized) {
    return "";
  }
  try {
    return normalizeAuthenticatedUrl(normalized, { errorCode: "invalid_iota_faucet_url" });
  } catch {
    throw new Error("invalid_iota_faucet_url");
  }
}

export function resolveIotaRpcUrl(options = {}, deps = {}) {
  const env = deps.env || process.env;
  const network = normalizeNetwork(resolveRequestedNetwork(options, env));
  const rpcUrl = normalizeRpcUrl(resolveRequestedRpcUrl(options, env));
  if (rpcUrl) {
    return {
...
export function resolveIotaFaucetUrl(options = {}, deps = {}) {
  const env = deps.env || process.env;
  const explicitUrl = normalizeFaucetUrl(
    normalizeString(options.faucetUrl) || firstNonEmptyEnvValue(IOTA_FAUCET_URL_ENV_NAMES, env),
  );
  if (explicitUrl) {
    return {
      network: normalizeNetwork(resolveRequestedNetwork(options, env)),
      faucetUrl: explicitUrl,
    };
  }
  const network = normalizeNetwork(resolveRequestedNetwork(options, env));
  if (network === "custom" || network === "mainnet" || network === "localnet") {
    throw new Error("missing_iota_faucet_url");
  }
  return {
    network,
    faucetUrl: getFaucetHost(network),
  };
}
...
export async function requestIotaFaucet(options = {}, deps = {}) {
  const faucetRequester = deps.requestIotaFromFaucetV0 || requestIotaFromFaucetV0;
  const env = deps.env || process.env;
  const rawAddress = normalizeString(options.address);
  const explicitAddress = normalizeIotaAddress(rawAddress);
  if (rawAddress && !explicitAddress) {
    throw new Error("invalid_iota_address");
  }
  const selected = explicitAddress
    ? { address: explicitAddress, alias: "" }
    : await resolveSelectedEntry(options, deps);
  const resolved = resolveIotaFaucetUrl(options, { env });
  const faucet = await faucetRequester({
    host: resolved.faucetUrl,
...
async function runBeforeBroadcastHook(beforeBroadcast) {
  if (beforeBroadcast === undefined || beforeBroadcast === null) {
    return;
  }
  if (typeof beforeBroadcast !== "function") {
    throw new Error("invalid_before_broadcast_hook");
  }
...
export async function executeIotaTransfer(options = {}, deps = {}) {
  ...
  if (typeof options.signature === "string" && options.signature.trim()) {
    ...
    await runBeforeBroadcastHook(options.beforeBroadcast);
