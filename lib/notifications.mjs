...
export const NOTIFICATION_PRESETS = Object.freeze({
  ...
  buyer: Object.freeze({
    description: "Buyer alerts for order state, milestones, dispute opening, and mailbox updates.",
    eventTypes: Object.freeze([
      "order.accepted",
      "order.status_changed",
      "milestone.submitted",
...
function acceptedOrderNextStep(payload) {
  ...
  const disputeBondPolicy =
    typeof payload?.disputeBondPolicy === "string" ? payload.disputeBondPolicy.trim() : "";

  if (status === "AWAITING_DEPOSITS") {
    if (disputeBondPolicy === "DUAL_BOND_REQUIRED") {
      return "Next step: fund the required dispute-bond deposits so the order can move into execution.";
    }
    if (disputeBondPolicy === "PLATFORM_FUNDED_MARKETING") {
      return "Next step: continue with the remaining deposit and escrow setup. The dispute bond is platform-funded for this order.";
    }
    return "Next step: complete the required deposits so the order can move into execution.";
  }
...
function acceptedOrderIntro(payload) {
  ...
  if (status === "AWAITING_DEPOSITS") {
    if (disputeBondPolicy === "PLATFORM_FUNDED_MARKETING") {
      return "Your bid was accepted. No user-funded dispute bond is required for this order.";
    }
