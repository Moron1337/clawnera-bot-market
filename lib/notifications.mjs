...
export const NOTIFICATION_PRESETS = Object.freeze({
  seller: Object.freeze({
    description: "Seller wallet inbox alerts for new bids, dispute opening, order state, milestones, and mailbox updates.",
  ...
  buyer: Object.freeze({
    description: "Buyer wallet inbox alerts for order state, milestones, dispute opening, and mailbox updates.",
    eventTypes: Object.freeze([
      "order.accepted",
      "order.mutual_cancel_approved",
      "order.status_changed",
  ...
  all: Object.freeze({
    description: "Broad wallet inbox alerts for both buy-side and sell-side workflows.",
    eventTypes: Object.freeze([
  ...
  mailbox: Object.freeze({
    description: "Legacy mailbox-only wallet inbox notifications.",
    eventTypes: Object.freeze(["mailbox.signal_posted"])
  })
});

export const SUPPORTED_NOTIFICATION_EVENT_TYPES = Object.freeze([
  "listing.created",
...
export const KNOWN_NOTIFICATION_EVENT_TYPES = Object.freeze([...SUPPORTED_NOTIFICATION_EVENT_TYPES].sort());
...
function eventHeader(eventType) {
  switch (eventType) {
    ...
    case "order.accepted":
      return "Clawnera bid accepted";
    case "order.mutual_cancel_approved":
      return "Clawnera mutual cancel";
    case "order.status_changed":
      return "Clawnera order update";
    case "order.deadline_extension_proposed":
    case "order.deadline_extended":
    case "order.deadline_extension_rejected":
    case "order.deadline_extension_expired":
      return "Clawnera deadline update";
    case "milestone.submitted":
    case "milestone.accepted":
    case "milestone.rejected":
      return "Clawnera milestone update";
    case "dispute.finalization_planned":
    case "dispute.escrow_resolution_planned":
      return "Clawnera dispute action";
    case "dispute.finalized":
    case "dispute.fallback_resolved":
      return "Clawnera dispute settled";
    case "reviewer.invited":
      return "Clawnera reviewer invite";
    case "reviewer.accepted":
    case "reviewer.vote_committed":
    case "reviewer.vote_revealed":
      return "Clawnera reviewer update";
    case "reputation.profile_created":
    case "reputation.participant_updated":
      return "Clawnera reputation update";
    case "mailbox.bound":
      return "Clawnera mailbox ready";
    case "mailbox.signal_acked":
      return "Clawnera mailbox ack";
    ...
    case "listing.deposit_locked":
    case "listing.deposit_settled":
      return "Clawnera listing deposit";
    default:
      return `Clawnera ${eventType || "event"}`;
  }
}

function acceptedOrderNextStep(payload) {
  const status = typeof payload?.status === "string" ? payload.status.trim() : "";

  if (status === "AWAITING_DEPOSITS") {
    return "Next step: fund the required dispute-bond deposits so the order can move into execution.";
  }

  return "";
}

function acceptedOrderIntro(payload) {
  const status = typeof payload?.status === "string" ? payload.status.trim() : "";
  if (status === "AWAITING_DEPOSITS") {
    return "Your bid was accepted. The order is now waiting for deposits.";
  }
...
export function formatNotificationEventForTelegram(event) {
  ...
  switch (event?.eventType) {
    ...
    case "order.accepted":
      bodyLines = [
        acceptedOrderIntro(payload),
        acceptedOrderNextStep(payload),
        ...genericDetails(payload, ["orderId", "listingId", "status"])
      ];
      break;
    case "order.mutual_cancel_approved":
      bodyLines = [
        "One party approved cooperative cancellation.",
        ...genericDetails(payload, ["orderId", "actor", "escrowObjectId"])
      ];
      break;
    case "order.status_changed":
      bodyLines = [
    ...
    case "order.deadline_extension_proposed":
      bodyLines = [
        ...
        ...genericDetails(payload, ["orderId", "extensionObjectId", "proposer", "currentDeadlineMs", "proposedDeadlineMs"])
      ];
      break;
    case "order.deadline_extended":
      bodyLines = [
        "Order deadline extended.",
        ...genericDetails(payload, ["orderId", "extensionObjectId", "acceptor", "newDeadlineMs"])
      ];
      break;
    case "order.deadline_extension_rejected":
    case "order.deadline_extension_expired":
      bodyLines = [
        event.eventType === "order.deadline_extension_rejected"
          ? "Deadline extension rejected."
          : "Deadline extension expired.",
        ...genericDetails(payload, ["orderId", "extensionObjectId", "actor", "finalState"])
      ];
      break;
    case "milestone.submitted":
    case "milestone.accepted":
    ...
    case "mailbox.signal_acked":
      bodyLines = [
        "Mailbox acknowledgement recorded.",
        ...genericDetails(payload, ["orderId", "ackedSeq", "ackerRole"])
      ];
      break;
    case "mailbox.bound":
      bodyLines = [
        "Mailbox binding is ready.",
        ...genericDetails(payload, ["orderId", "mailboxObjectId"])
      ];
      break;
    case "dispute.opened":
      bodyLines = [
        "Dispute open plan is ready. Re-read the order and dispute state after the related write completes.",
        ...genericDetails(payload, ["mode", "orderId", "disputeCaseId", "status", "resolution"])
      ];
      break;
    case "dispute.finalization_planned":
      bodyLines = [
        "Dispute finalize plan is ready.",
        ...genericDetails(payload, ["orderId", "disputeCaseId", "resolutionPath"])
      ];
      break;
    case "dispute.escrow_resolution_planned":
      bodyLines = [
        "Dispute escrow resolution plan is ready.",
        ...genericDetails(payload, ["orderId", "disputeCaseId", "escrowObjectId"])
      ];
      break;
    case "dispute.finalized":
      bodyLines = [
        "Dispute finalized on-chain.",
        ...genericDetails(payload, ["orderId", "disputeCaseId", "winningSide", "resolution"])
      ];
      break;
    case "dispute.fallback_resolved":
      bodyLines = [
        "Dispute fallback resolved on-chain.",
        ...genericDetails(payload, ["orderId", "disputeCaseId", "resolution", "actor"])
      ];
      break;
    case "reviewer.invited":
      bodyLines = [
        "A reviewer invite is now live for this wallet.",
        ...genericDetails(payload, ["disputeCaseObjectId", "assignmentRound", "invitedAtMs"])
      ];
      break;
    case "reviewer.accepted":
      bodyLines = [
        "Reviewer accepted an invite.",
        ...genericDetails(payload, ["disputeCaseObjectId", "reviewer", "assignmentRound"])
      ];
      break;
    case "reviewer.vote_committed":
      bodyLines = [
        "Reviewer vote commitment recorded.",
        ...genericDetails(payload, ["disputeCaseObjectId", "reviewer", "assignmentRound"])
      ];
      break;
    case "reviewer.vote_revealed":
      bodyLines = [
        "Reviewer vote reveal recorded.",
        ...genericDetails(payload, ["disputeCaseObjectId", "reviewer", "vote", "assignmentRound"])
      ];
      break;
    case "reputation.profile_created":
      bodyLines = [
        "Reputation profile created.",
        ...genericDetails(payload, ["address", "profileObjectId", "profileStatus"])
      ];
      break;
    case "reputation.participant_updated":
      bodyLines = [
        "Reputation participant summary updated.",
        ...genericDetails(payload, ["address", "orderId", "outcome"])
      ];
      break;
    case "listing.deposit_locked":
      bodyLines = [
        "Listing deposit locked.",
        ...genericDetails(payload, ["listingId", "listingDepositObjectId", "amount"])
      ];
      break;
    case "listing.deposit_settled":
      bodyLines = [
        "Listing deposit settled.",
        ...genericDetails(payload, ["listingId", "listingDepositObjectId", "settlementMode"])
      ];
      break;
    case "listing.status_changed":
      bodyLines = [
