...
export function normalizeAuthState(input, apiBaseFallback = "") {
  ...
  const apiBase = normalizeString(record.apiBase || apiBaseFallback);
  ...
  const expiresAtMs = Number.isFinite(Number(record.expiresAtMs)) ? Number(record.expiresAtMs) : null;
  const refreshExpiresAtMs = Number.isFinite(Number(sessionRecord.refreshExpiresAtMs))
    ? Number(sessionRecord.refreshExpiresAtMs)
    : Number.isFinite(Number(record.refreshExpiresAtMs))
      ? Number(record.refreshExpiresAtMs)
      : null;
  ...
  return {
    ...
    session: {
      ...
      refreshExpiresAtMs,
      lastRefreshedAtMs: Number.isFinite(Number(sessionRecord.lastRefreshedAtMs))
        ? Number(sessionRecord.lastRefreshedAtMs)
        : null
    },
