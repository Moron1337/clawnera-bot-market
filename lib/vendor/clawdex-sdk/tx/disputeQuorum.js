...
export function buildFinalizeDisputeCaseTx(req) {
    ...
    const disputeQuorumConfig = tx.object(disputeQuorumConfigObjectId);
    tx.moveCall({
        target: disputeQuorumTarget(packageId, bondCoinType === undefined ? "finalize_case_with_quorum" : "finalize_case_with_typed_quorum"),
        typeArguments: bondCoinType === undefined ? undefined : [bondCoinType],
        arguments: [
            tx.object(disputeCaseObjectId),
            tx.object(bondObjectId),
            tx.object(reviewerRegistryObjectId),
    ...
    tx.moveCall({
        ...
        typeArguments: [escrowCoinType],
        arguments: [disputeQuorumConfig, tx.object(escrowObjectId)]
    });
    return tx;
}
export function buildResolveDisputeTimeoutFallbackTx(req) {
    ...
    const disputeQuorumConfig = tx.object(disputeQuorumConfigObjectId);
    tx.moveCall({
        target: disputeQuorumTarget(packageId, bondCoinType === undefined
