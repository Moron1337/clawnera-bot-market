...
import { assertIotaDisputeBondSettlementAsset, assertByteVectorInput, assertCanonicalProtocolString, assertPositiveAmount, assertValidIotaAddress, assertValidIotaObjectId, assertValidMoveTypeTag } from "../validation.js";
...
function validatedOptionalBondCoinType(input) {
    return input === undefined ? undefined : assertValidMoveTypeTag(input, "bond_coin_type");
}
...
export function buildRegisterBootstrapReviewerTx(req) {
    const { tx, packageId } = buildBaseTx(req);
    const governanceConfigObjectId = assertValidIotaObjectId(req.governanceConfigObjectId, "governance_config_object_id");
    const reviewerRegistryObjectId = assertValidIotaObjectId(req.reviewerRegistryObjectId, "reviewer_registry_object_id");
    const disputeQuorumConfigObjectId = assertValidIotaObjectId(req.disputeQuorumConfigObjectId, "dispute_quorum_config_object_id");
    ...
    const stakeAmount = assertPositiveAmount(req.stakeAmount, "stake_amount");
    const clockObjectId = validatedClockObjectId(req.clockObjectId);
    const stakeSource = req.paymentCoinObjectId === undefined
        ? tx.gas
        : tx.object(assertValidIotaObjectId(req.paymentCoinObjectId, "payment_coin_object_id"));
    const stakeCoin = tx.splitCoins(stakeSource, [tx.pure.u64(stakeAmount)]);
    tx.moveCall({
        target: disputeQuorumTarget(packageId, "register_bootstrap_reviewer_entry"),
        arguments: [
            tx.object(governanceConfigObjectId),
            tx.object(reviewerRegistryObjectId),
            tx.object(disputeQuorumConfigObjectId),
...
export function buildInitOrderDisputeBondTx(req) {
    ...
    const clockObjectId = validatedClockObjectId(req.clockObjectId);
    tx.moveCall({
        target: orderEscrowTarget(packageId, "init_order_dispute_bond_for_escrow"),
        arguments: [
            tx.object(governanceConfigObjectId),
...
export function buildInitOrderDisputeBondTypedTx(req) {
    ...
    const clockObjectId = validatedClockObjectId(req.clockObjectId);
    tx.moveCall({
        target: orderEscrowTarget(packageId, "init_order_dispute_typed_bond_for_escrow"),
...
function buildFundOrderDisputeBondTx(req, fn) {
    ...
    const expectedConfigRevision = assertU64(req.expectedConfigRevision, "expected_config_revision");
    const expectedEconomicsRevision = assertU64(req.expectedEconomicsRevision, "expected_economics_revision");
    ...
    tx.moveCall({
        target: orderEscrowTarget(packageId, fn),
        arguments: [
            tx.object(governanceConfigObjectId),
            tx.object(escrowObjectId),
            tx.object(bondObjectId),
            tx.object(disputeQuorumConfigObjectId),
            tx.pure.u64(expectedConfigRevision),
            tx.pure.u64(expectedEconomicsRevision),
            paymentCoin,
            tx.object(clockObjectId)
        ]
    });
...
export function buildFundOrderDisputeBondAsBuyerTx(req) {
    return buildFundOrderDisputeBondTx(req, "fund_dispute_bond_as_buyer");
}
export function buildFundOrderDisputeBondAsSellerTx(req) {
    return buildFundOrderDisputeBondTx(req, "fund_dispute_bond_as_seller");
}
function buildFundOrderDisputeBondTypedTx(req, fn) {
    ...
    const expectedConfigRevision = assertU64(req.expectedConfigRevision, "expected_config_revision");
    const expectedEconomicsRevision = assertU64(req.expectedEconomicsRevision, "expected_economics_revision");
    if (typeof req.expectedInviteOnlyReviewers !== "boolean") {
        throw new Error("invalid_expected_invite_only_reviewers");
    }
    assertValidBondAmount(amount, req.bondAmountPolicy);
    const clockObjectId = validatedClockObjectId(req.clockObjectId);
    ...
    tx.moveCall({
        ...
        arguments: [
            ...
            tx.pure.u64(expectedConfigRevision),
            tx.pure.u64(expectedEconomicsRevision),
            tx.pure.bool(req.expectedInviteOnlyReviewers),
            paymentCoin,
            tx.object(clockObjectId)
        ]
    });
...
export function buildFundOrderDisputeBondTypedAsBuyerTx(req) {
    return buildFundOrderDisputeBondTypedTx(req, "fund_typed_dispute_bond_as_buyer");
}
export function buildFundOrderDisputeBondTypedAsSellerTx(req) {
    return buildFundOrderDisputeBondTypedTx(req, "fund_typed_dispute_bond_as_seller");
}
export function buildCancelPendingOrderDisputeBondTx(req) {
    ...
    const bondObjectId = assertValidIotaObjectId(req.bondObjectId, "bond_object_id");
    tx.moveCall({
        target: orderEscrowTarget(packageId, "cancel_pending_dispute_bond"),
        arguments: [
            tx.object(disputeQuorumConfigObjectId),
            tx.object(escrowObjectId),
            tx.object(bondObjectId)
...
export function buildCancelPendingOrderDisputeTypedBondTx(req) {
    ...
    const bondObjectId = assertValidIotaObjectId(req.bondObjectId, "bond_object_id");
    tx.moveCall({
        target: orderEscrowTarget(packageId, "cancel_pending_typed_dispute_bond"),
        typeArguments: [coinType],
        arguments: [
            tx.object(disputeQuorumConfigObjectId),
            tx.object(escrowObjectId),
...
export function buildAuthorizeReplacementReviewerSelectionTx(req) {
    ...
    const reviewers = req.invitedReviewerAddresses.map((value) => assertValidIotaAddress(value, "invited_reviewer_address"));
    tx.moveCall({
        target: disputeQuorumTarget(packageId, "authorize_replacement_reviewer_selection"),
        arguments: [
            tx.object(governanceConfigObjectId),
...
export function buildOpenMilestoneDisputeCaseTx(req) {
    const { tx, packageId } = buildBaseTx(req);
    assertSupportedOpenDisputeArgMode(req.openDisputeArgMode);
    const milestoneId = assertCanonicalProtocolString(req.milestoneId, "milestone_id", MAX_MILESTONE_ID_LEN);
...
export function buildFinalizeDisputeCaseTx(req) {
    ...
    const disputeCaseObjectId = assertValidIotaObjectId(req.disputeCaseObjectId, "dispute_case_object_id");
    const bondObjectId = assertValidIotaObjectId(req.bondObjectId, "bond_object_id");
    const bondCoinType = validatedOptionalBondCoinType(req.bondCoinType);
    const reviewerRegistryObjectId = assertValidIotaObjectId(req.reviewerRegistryObjectId, "reviewer_registry_object_id");
    const disputeQuorumConfigObjectId = assertValidIotaObjectId(req.disputeQuorumConfigObjectId, "dispute_quorum_config_object_id");
    const clockObjectId = validatedClockObjectId(req.clockObjectId);
    const escrowObjectId = validatedEscrowObjectId(req.escrowObjectId);
    ...
    assertIotaDisputeBondSettlementAsset(escrowCoinType, bondCoinType);
    tx.moveCall({
        target: orderEscrowTarget(packageId, bondCoinType === undefined
            ? "finalize_case_with_quorum_and_resolve_escrow"
            : "finalize_case_with_typed_quorum_and_resolve_escrow"),
        typeArguments: bondCoinType === undefined ? [escrowCoinType] : [escrowCoinType, bondCoinType],
        arguments: [
            tx.object(disputeCaseObjectId),
            tx.object(bondObjectId),
            tx.object(reviewerRegistryObjectId),
