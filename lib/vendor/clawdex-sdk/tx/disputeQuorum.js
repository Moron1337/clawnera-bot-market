...
export function buildAuthorizeOrderReviewerSelectionTx(req) {
    ...
    tx.moveCall({
        ...
        arguments: hasInvites
            ? [tx.object(selector), tx.object(cfg), tx.object(registry), tx.object(bond), tx.pure.string(milestoneId), tx.pure.address(escrowId), tx.pure.address(intendedParty), tx.pure.vector("address", reviewers), tx.pure.u64(expiresAtMs), tx.object(clock)]
            : [tx.object(selector), tx.object(cfg), tx.object(registry), tx.object(bond), tx.pure.string(milestoneId), tx.pure.address(escrowId), tx.pure.address(intendedParty), tx.pure.u64(expiresAtMs), tx.object(clock)]
    });
    return tx;
}
