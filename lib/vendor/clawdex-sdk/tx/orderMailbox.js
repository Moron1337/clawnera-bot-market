...
function normalizeAckedSeq(value) {
    ...
    return assertPositiveAmount(BigInt(normalized), "acked_seq");
}
export function buildInitOrderMailboxTx(req) {
    ...
    if (req.settlementAbi !== "legacy" && req.settlementAbi !== "fresh") {
        throw new Error("iota_order_mailbox_settlement_abi_required");
    }
    const arguments_ = [
    ...
    if (req.settlementAbi === "fresh") {
        arguments_.push(tx.object(assertValidIotaObjectId(req.orderMailboxRegistryObjectId, "order_mailbox_registry_object_id")));
    }
    arguments_.push(tx.object(clockObjectId));
    tx.moveCall({
...
export function buildPostOrderMailboxSignalTx(req) {
    ...
    const arguments_ = [];
    if (req.settlementAbi === "fresh") {
        arguments_.push(tx.object(assertValidIotaObjectId(req.governanceConfigObjectId ?? "", "governance_config_object_id")));
    }
    arguments_.push(tx.object(mailboxObjectId), tx.pure.u8(signalType), tx.pure.string(ciphertextHash), tx.pure.string(payloadRef), tx.object(clockObjectId));
    tx.moveCall({ target: orderMailboxTarget(packageId, "post_signal"), arguments: arguments_ });
    return tx;
}
...
export function buildAckOrderMailboxSignalTx(req) {
    if (req.settlementAbi !== "legacy") {
        throw new Error("iota_order_mailbox_acknowledgement_not_supported");
    }
    const { tx, packageId } = buildBaseTx(req);
    const mailboxObjectId = assertValidIotaObjectId(req.mailboxObjectId, "mailbox_object_id");
...
export function buildDeleteClosedOrderMailboxTx(req) {
    void req;
    throw new Error("iota_order_mailbox_deletion_not_supported");
}
export function buildDeleteClosedOrderMailboxGuardedTx(req) {
    if (req.settlementAbi !== "legacy") {
        throw new Error("iota_order_mailbox_deletion_not_supported");
    }
    const { tx, packageId } = buildBaseTx(req);
    ...
    const governanceConfigObjectId = assertValidIotaObjectId(req.governanceConfigObjectId, "governance_config_object_id");
    tx.moveCall({
        target: orderMailboxTarget(packageId, "delete_closed_mailbox_guarded"),
        arguments: [tx.object(governanceConfigObjectId), tx.object(mailboxObjectId)]
    });
    return tx;
}
