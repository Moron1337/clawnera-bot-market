...
import { isValidStructTag, isValidSuiAddress, isValidSuiObjectId, normalizeStructTag, normalizeSuiAddress, normalizeSuiObjectId, } from "@mysten/sui/utils";
...
const SUI_SHORT_OR_CANONICAL_HEX_ID_PATTERN = /^0x[0-9a-fA-F]{1,64}$/;
const utf8Encoder = new TextEncoder();
const ZERO_IOTA_ADDRESS = `0x${"0".repeat(64)}`;
const ZERO_SUI_ADDRESS = `0x${"0".repeat(64)}`;
export const IOTA_NATIVE_COIN_TYPE = `0x${"0".repeat(63)}2::iota::IOTA`;
...
export function assertValidSuiAddress(input, fieldName) {
    if (typeof input !== "string" || !SUI_SHORT_OR_CANONICAL_HEX_ID_PATTERN.test(input)) {
        throw new Error(`invalid_${fieldName}`);
    }
    const normalized = normalizeSuiAddress(input);
    if (!isValidSuiAddress(normalized)) {
        throw new Error(`invalid_${fieldName}`);
    }
    return normalized;
}
export function assertValidSuiObjectId(input, fieldName) {
    if (typeof input !== "string" || !SUI_SHORT_OR_CANONICAL_HEX_ID_PATTERN.test(input)) {
        throw new Error(`invalid_${fieldName}`);
    }
    const normalized = normalizeSuiObjectId(input);
    if (!isValidSuiObjectId(normalized)) {
        throw new Error(`invalid_${fieldName}`);
    }
    return normalized;
}
export function assertDistinctNonZeroSuiAddresses(firstInput, firstFieldName, secondInput, secondFieldName, errorFieldName) {
    const first = assertValidSuiAddress(firstInput, firstFieldName);
    const second = assertValidSuiAddress(secondInput, secondFieldName);
    if (first === ZERO_SUI_ADDRESS || second === ZERO_SUI_ADDRESS || first === second) {
        throw new Error(`invalid_${errorFieldName}`);
    }
    return { first, second };
}
export function assertValidSuiMoveStructTag(input, fieldName) {
    if (typeof input !== "string") {
        throw new Error(`invalid_${fieldName}`);
    }
    const normalizedInput = input.trim();
    if (!normalizedInput || /\s/.test(normalizedInput) || normalizedInput.length > MAX_MOVE_TYPE_TAG_LENGTH) {
        throw new Error(`invalid_${fieldName}`);
    }
    if (!isValidStructTag(normalizedInput)) {
        throw new Error(`invalid_${fieldName}`);
    }
    return normalizeStructTag(normalizedInput);
}
...
export function assertIotaDisputeBondSettlementAsset(escrowCoinType, bondCoinType) {
    if (bondCoinType === undefined) {
        if (escrowCoinType !== IOTA_NATIVE_COIN_TYPE) {
            throw new Error("native_dispute_bond_requires_iota_escrow_coin_type");
        }
        return;
    }
    if (bondCoinType !== escrowCoinType) {
        throw new Error("typed_bond_coin_type_must_match_escrow_coin_type");
    }
    if (bondCoinType === IOTA_NATIVE_COIN_TYPE) {
        throw new Error("typed_dispute_bond_requires_non_iota_asset");
    }
}
...
export function isValidSuiAddressInput(input) {
    if (typeof input !== "string" || !SUI_SHORT_OR_CANONICAL_HEX_ID_PATTERN.test(input)) {
        return false;
    }
    const normalized = normalizeSuiAddress(input);
    return isValidSuiAddress(normalized);
}
export function isValidSuiObjectIdInput(input) {
    if (typeof input !== "string" || !SUI_SHORT_OR_CANONICAL_HEX_ID_PATTERN.test(input)) {
        return false;
    }
    const normalized = normalizeSuiObjectId(input);
    return isValidSuiObjectId(normalized);
}
...
export function assertCanonicalOrderRef(input, fieldName, maxLength) {
    const value = assertCanonicalProtocolString(input, fieldName, maxLength);
