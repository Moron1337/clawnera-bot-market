...
test("help command prints usage", () => {
  ...
  assert.doesNotMatch(result.stdout, /Available journeys:/);
  assert.doesNotMatch(result.stdout, /Available recipes:/);
  assert.doesNotMatch(result.stdout, /clawnera-help auth-login/);
});

test("help all prints full inventory and indexes", () => {
  ...
  assert.match(result.stdout, /Available topics:/);
...
test("recipe compact output focuses on immediate command, readback, and next", () => {
  ...
  assert.match(result.stdout, /--listing-deposit-object-id <listingDepositObjectId>/);
  assert.match(result.stdout, /^write:POST \/listings/m);
  assert.match(result.stdout, /^read:GET \/compliance\/me \| GET \/listings\/\{listingId\}/m);
