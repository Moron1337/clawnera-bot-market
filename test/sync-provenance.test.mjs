...
test("sync script uses exact commit blobs and an isolated build snapshot for the exact mapping set", () => {
  ...
  assert.equal(committedMappings.length, 16);
  ...
  for (const retiredPath of RETIRED_V4_SYNC_PATHS) {
    assert.ok(source.includes(`  "${retiredPath}"`), `retired v4 destination missing: ${retiredPath}`);
    assert.ok(!mappings.some(({ destination }) => destination === retiredPath));
  }
});
...
test("checked-in sync provenance has the exact unique source set", () => {
  ...
  assert.ok(RETIRED_V4_SYNC_PATHS.every((entry) => !EXPECTED_SYNC_PATHS.includes(entry)));
...
test("v4 sync provenance requires all five Fresh IOTA callable-surface roots", async () => {
  ...
  try {
    ...
    for (const retiredPath of RETIRED_V4_SYNC_PATHS) {
      const retiredTarget = path.join(temporaryRoot, retiredPath);
      await fsPromises.mkdir(path.dirname(retiredTarget), { recursive: true });
      await fsPromises.writeFile(retiredTarget, "legacy\n");
      assert.throws(
        () => validateSyncProvenance({ rootDir: temporaryRoot, manifestText: manifest }),
        new RegExp(`retired_sync_path_present:${retiredPath.replaceAll(".", "\\.")}`),
      );
      await fsPromises.rm(retiredTarget);
    }
  } finally {
...
test("sync manifest rejects duplicate fields, duplicate hashes, and path-set drift", () => {
  ...
  assert.throws(
    () => parseSyncManifest(manifest.replace(
      `format=${CURRENT_SYNC_FORMAT}\n`,
      `format=${CURRENT_SYNC_FORMAT}\nformat=${CURRENT_SYNC_FORMAT}\n`,
    )),
